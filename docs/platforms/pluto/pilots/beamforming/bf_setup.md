...
## Hardware
- **ADALM-PLUTO** ({ref}`modified for 2T2R <adding-a-second-rx-and-tx-channel>`) — the latest revision (Rev C) supports a second receive path (via u.FL to SMA) when configured as an **AD9361** rather than AD9363.  
...
- Make sure you've [installed the drivers](../../getStarted/drivers.md) and followed the [Pluto Python](../../software/python_install.md) setup instructions.

## Setup  

1. Configure the Pluto for dual Rx (Rev C only) by following {ref}`this guide <adding-a-second-rx-and-tx-channel>`.
