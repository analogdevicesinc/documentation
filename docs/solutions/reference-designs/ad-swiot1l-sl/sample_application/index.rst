...
- Python 3.10 or newer. (Refer to pyadi-iio documentation for currently
  supported versions.)

- Git command-line tools installed.

- Raspberry Pi with :adi:`kuiper-linux` image installed. Follow the instructions in
  the :ref:`kuiper` documentation to prepare the Raspberry Pi.
...
3. Connect the :adi:`TMP01` temperature sensor to the first :adi:`AD-SWIOT1L-SL` board
   ...
   - **Channel 4 (CH4)** is configured as a **Voltage Input** to measure the
     ...
     - ``CH4 SWIO`` → ``TMP01 VPTAT``
     - ``CH4 GND`` → ``TMP01 GND``

   .. note::

      VPTAT is a voltage output that is proportional to absolute temperature.
      The temperature in degrees Celsius is then calculated in software.
