.. _ad_fmcadc2_ebz quickstart_guide:
...
.. _ad_fmcadc2_ebz vc707_quickstart:

Linux Quick Start (VC707)
-------------------------------------------------------------------------------

This guide provides instructions on how to set up the AD-FMCADC2-EBZ on:
...
-  Xilinx VC707 / ZC706 FPGA carrier board.
...
Download the pre-build image for your device in question: `Master Microblaze images <https://swdownloads.analog.com/cse/microblaze/master/latest_microblaze_images.tar.gz>`_
...
Below is just an example and the file-names may vary.

.. collapsible:: XMD command console
...
run the XSCT terminal from Start menu -> Xilinx Design Tools -> Xilinx Software
Command Line Tool. In Linux, source the settings.sh file first.

.. collapsible:: XSCT command console
...
The second method is to run the TCL script which handles loading the bit file
and the Linux image. Run Vivado TCL Shell from Windows Start menu -> Xilinx
Design Suite -> Vivado -> Vivado TCL Shell. In Linux, source the settings.sh
file first.

Then run the TCL script:

.. collapsible:: Vivado TCL shell or Linux console
...
Connect a USB cable to the USB UART port (115200, 8N1) to access the Linux
console.
...
If the FPGA carrier board features an LCD display and the board is connected to
a DHCP-enabled network, the IP address will be printed on the display, allowing
remote access via SSH or libiio.

If the IP address shown is 192.168.2.2, DHCP failed and the board is using
its default address. This address may not be within your subnet, so remote
access may not work.
...
.. figure:: ../images/ml605-lcd.png
   ...
   LCD display showing IP address

The kernel start-up messages should appear as follows:
...
from it. See :ref:`iio-oscilloscope` for more information.

Build and start the oscilloscope on a network-enabled Linux host. Once the
application is launched, go to Settings -> Connect and enter the IP address of
the target in the popup window.

.. _ad_fmcadc2_ebz baremetal_quickstart:

No-OS Quick Start (ZC706 / VC707)
-------------------------------------------------------------------------------
...
.. admonition:: Download
   ...
   -  AD-FMCADC2-EBZ no-OS - :git-no-OS:`fmcadc2 project <2018_R1:fmcadc2>`

Required Software
~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~

-  The Xilinx tools version supported on each release can be found in the
   :git-hdl:`HDL repository <hdl_2021_r2:projects/fmcadc2>`.
-  A UART terminal (Tera Term/Hyperterminal), baud rate 115200.

Software Setup
~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~

-  After :external+hdl:ref:`build_hdl`, open the Xilinx SDK.

-  When the SDK starts it asks to provide a folder where to store the
   workspace. Any folder can be provided.
...
.. figure:: ../images/new_empty_project.png
   :align: center
   :width: 500

-  Download the required source files from the Downloads section above.

-  Copy the downloaded source files into the **src** folder of the just
   created empty application.

.. figure:: ../images/complete_project.png
   :align: center
   :width: 500

-  The project can be easily debugged using the Vivado Hardware Manager and
   the integrated logic analyzer (ILA) debug cores.

.. figure:: ../images/hardware_manager.png
   :align: center
   :width: 700
