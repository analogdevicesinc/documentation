...
The ADRV904x provides a RAM capture feature for capturing raw RX/ORX ADC
samples at various points in the signal chain. This is useful for debugging,
signal analysis, and calibration verification.

The ``adrv904x-ramc`` IIO device exposes this feature with standard IIO buffer
...
.. note::
   ...
   RX data streaming, use the JESD interface (``axi-adrv904x-rx-hpc``).
...
``adrv904x-ramc``. It provides 10 channel pairs (RX0-RX7 and ORX0-ORX1), each
...
2. In the **Plot Channels** panel, expand ``adrv904x-ramc``.
...
.. code-block:: bash
   ...
   iio_readdev -u <uri> -s 1024 adrv904x-ramc voltage0_i voltage0_q > capture.bin

Capture from RX2:

.. code-block:: bash

   iio_readdev -u <uri> -s 1024 adrv904x-ramc voltage2_i voltage2_q > capture.bin
