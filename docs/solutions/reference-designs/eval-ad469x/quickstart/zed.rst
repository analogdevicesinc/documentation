...
#. The blue LD12/DONE LED and red LD0 LED should start blinking ~20-30 seconds
   ...
   .. tip::
      ...
      Linux versions prior to ADI Kuiper Linux for Evaluation
      version 2024-8-27 will instead boot with the BLUE LD12/DONE
      LED blinking immediately and LD7 blinking after ~20-30
      seconds. This may indicate that an improved version of the
      ACE plugin is available if the SD card is updated to the
      latest version.
...
data captured from the AD4696 evaluation board.
...
- :git-no-os:`projects/ad469x_evb`. More no-OS build
...
- A UART terminal (Putty/Tera Term/Minicom, etc.), Baud rate
  115200 (8N1)
...
connecting the AD469x evaluation board to the ZedBoard, configuring
...
#. Configure ZedBoard for JTAG boot:
   ...
   set for JTAG boot mode. VADJ must be set to 1.8 V, as specified
   in the HDL project's README at :external+hdl:ref:`ad469x_evb`.
