...
1. Once done with the installation or an update of the latest IIO-Oscilloscope,
...
3. After the board is connected, select the **one-bit-adc-dac-device**, which is
...
4. Input sources for AD3552R devices axi-ad3552r-0 and axi-ad3552r-1 can be
...
Select the desired input source for both AD3552R devices axi-ad3552r-0 and axi-ad3552r-1 as dma_input. .. figure:: https://wiki.analog.com/_media/resources/eval/user-guides/circuits-from-the-lab/cn0585/input_source_dac0_cn0585.png
   :width: 700px
...
5. Select the desired output range for both AD3552R devices. .. figure:: https://wiki.analog.com/_media/resources/eval/user-guides/circuits-from-the-lab/cn0585/output_range_cn0585.png
   :width: 700px

  Figure 12. AD3552R Output Range Selection in IIO Oscilloscope* .. warning::
...
6. From the DAC Data Manager Window select the output channels of the DAC and
   enable the cyclic buffer for each DAC.

7. Load an example file (.mat, .txt, etc) from the IIO Oscilloscope installation
...
9. From the Debug window, select the stream_status IIO Attribute and start the stream (start_stream_synced means that all 4 channels are updated at the same time and the data streaming process waits for both DACs to be started). .. figure:: https://wiki.analog.com/_media/resources/eval/user-guides/circuits-from-the-lab/cn0585/stream_status_iio.jpg

   Figure 14. DAC Stream Status Selection* .. important::

      If the source is set as dma_input and the data from all 4 channels needs to be synchronized make sure that you write the start_stream_synced for the

      axi-ad3552r-1 device first then for axi-ad3552r-0.

10. After the stream_status has been written and 4 channels are enabled, hit play button. Then data capture window can be seen like in Figure 15. .. figure:: https://wiki.analog.com/_media/resources/eval/user-guides/circuits-from-the-lab/cn0585/captured_loopback_signal_cn0585.jpg
   :width: 700px
...
1. Download or git clone
   `pyadi-iio repository <https://github.com/analogdevicesinc/pyadi-iio.git>`__
   to your local drive.
...
3. After installing and configuring PYADI-IIO on your machine, you are now ready
...
1. Ensure MATLAB package ``Communications Toolbox Support Package for Analog
   Devices ADALM-Pluto Radio`` is installed from Matlab Add-Ons.

2. Execute the following commands on the computer terminal (Cygwin is
...
4. Open CN0585StreamingTest.m from the test subdirectory, update the board_ip
