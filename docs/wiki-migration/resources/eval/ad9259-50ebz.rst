...
| This user guide describes the :adi:`AD9228`, :adi:`AD9259` and :adi:`AD9287` evaluation boards, :adi:`AD9228-65KITZ <AD9228>`, :adi:`AD9259-50KITZ <AD9259>` and :adi:`AD9287-100KITZ <AD9287>`, which provide all of the support circuitry required to operate these parts in their various modes and configurations. The application software used to interface with the devices is also described.
| The :adi:`AD9228`, :adi:`AD9259` and :adi:`AD9287` data sheets provide additional information and should be consulted when using the evaluation board. All documents and software tools are available at :adi:`www.analog.com/hsadcevalboard <hsadcevalboard>`. For additional information or questions, send an email to highspeed.converters@analog.com.
...
.. container:: centeralign
   ...
   *Figure 1. Evaluation Board Connection—*\ :adi:`AD9228-65KITZ <AD9228>`\ *,* :adi:`AD9259-50KITZ <AD9259>` *or* :adi:`AD9287-100KITZ <AD9287>` *(on Left) and* :adi:`HSC-ADC-EVALCZ <hsadcevalboard>` *(on Right)*


Helpful Documents
-----------------

-  :adi:`AD9228`, :adi:`AD9259` or :adi:`AD9287` data sheet
...
-  Schematics: `AD9228-65EBZ RevB <https://wiki.analog.com/_media/resources/eval/ad9228-65ebz_revb_schematic.pdf>`__, `HSC-ADC-FIFO5-INTZ RevA <https://wiki.analog.com/_media/resources/eval/hsc-adc-fifo5-intz_reva_schematic.pdf>`__
-  Bill of Materials: `AD9228-65EBZ RevB <https://wiki.analog.com/_media/resources/eval/ad9228-65ebz_revb_bom.xls>`__, `HSC-ADC-FIFO5-INTZ RevA <https://wiki.analog.com/_media/resources/eval/hsc-adc-fifo5-intz_reva_bom.xls>`__
-  PCB Gerber files: `AD9228-65EBZ RevB <https://wiki.analog.com/_media/resources/eval/ad9228-65ebz_revb_gerber.zip>`__, `HSC-ADC-FIFO5-INTZ RevA <https://wiki.analog.com/_media/resources/eval/hsc-adc-fifo5_intz_reva_gerber.zip>`__
...
-  :adi:`AD9228-65KITZ <AD9228>`, :adi:`AD9259-50KITZ <AD9259>` or :adi:`AD9287-100KITZ <AD9287>`
-  :adi:`HSC-ADC-EVALCZ <hsadcevalboard>` FPGA-based data capture kit

Getting Started
---------------

| This section provides quick start procedures for using the :adi:`AD9228-65KITZ <AD9228>`, :adi:`AD9259-50KITZ <AD9259>` or :adi:`AD9287-100KITZ <AD9287>` board. Both the default and optional settings are described.
...
-  Connect the boards to each other as shown in Figure 1.
-  Connect one 6 V, 2 A switching power supply (such as the CUI KSAFD0500300W1US that is supplied) to P503 on the :adi:`AD9228-65KITZ <AD9228>`, :adi:`AD9259-50KITZ <AD9259>` or :adi:`AD9287-100KITZ <AD9287>` evaluation board.
-  Connect one 6 V, 2 A switching power supply (such as the supplied CUI KSAFD0500300W1US) to J4 on the :adi:`HSC-ADC-EVALCZ <hsadcevalboard>` data capture board.
-  Connect the :adi:`HSC-ADC-EVALCZ <hsadcevalboard>` board (J6) to the PC using a USB cable.
-  On the ADC evaluation board, place jumpers on all four pin pairs of J301-304 to connect the SPI bus.
...
| The evaluation board provides the support circuitry required to operate the :adi:`AD9228`, :adi:`AD9259` and :adi:`AD9287` in their various modes and configurations. The converter can be driven differentially using a transformer (default) or an :adi:`AD8332` driver. The ADC can also be driven in a single-ended fashion. Separate power pins are provided to isolate the DUT from the drive circuitry of :adi:`AD8332`. Each input configuration can be selected by changing the connection of various jumpers.
| Figure 1 shows the typical bench characterization setup used to evaluate the ac performance of :adi:`AD9228`, :adi:`AD9259` and :adi:`AD9287`. It is critical that the signal sources used for the analog input and clock have very low phase noise (<1 ps rms jitter) to realize the optimum performance of the converter. Proper filtering of the analog input signal to remove harmonics and lower the integrated or broadband noise at the input is also necessary to achieve the specified noise performance.
| See the :doc:`Getting Started </wiki-migration/resources/eval/ad9259-50ebz>` section to get started, and visit :doc:`Design and Integration Files </wiki-migration/resources/eval/ad9259-50ebz>` for the complete schematics and layout diagrams. These diagrams demonstrate the routing and grounding techniques that should be applied at the system level when designing application boards using these converters.
...
| VREF is set to 1.0 V by tying the SENSE pin to ground, R237. This causes the ADC to operate in 2.0 V p-p full-scale range. A separate external reference option using the :adi:`ADR510` is also included on the evaluation board. Populate R231 and R235 and remove C214. See Voltage Reference section of the datasheet for proper use of the VREF options.

RBIAS
~~~~~

| RBIAS has a default setting of 10 kΩ (R201) to ground and is used to set the ADC core bias current.
| ==== Clock Circuitry==== The default clock input circuit on the evaluation board uses a simple transformer-coupled circuit using a high bandwidth 1:1 impedance ratio transformer (T201) that adds a low amount of jitter to the clock path. The clock input is 50 Ω terminated and ac-coupled to handle single-ended sine wave types of inputs. The transformer converts the single-ended input to a differential signal that is clipped before entering the ADC clock inputs.

| A differential LVPECL clock can also be used to clock the ADC input using the :adi:`AD9515` (U202). Populate R225 and R227 with 0 Ω resistors and remove R217 and R218 to disconnect the default clock path inputs. In addition, populate C207 and C208 with a 0.1 μF capacitor and remove C210 and C211 to disconnect the default clock path outputs. The :adi:`AD9515` has many pin-strappable options that are set to a default mode of operation. Consult the :adi:`AD9515` datasheet for more information about these and other options.
...
| The following is a brief description of the alternative analog input drive configuration using the :adi:`AD8332` dual VGA. If this drive options is in use, some components may need to be populated, in which case all the necessary components are listed in Bill of Materials under :doc:`Design and Integration Files </wiki-migration/resources/eval/ad9259-50ebz>` section. For more details on the AD8332 dual VGA, including how it works and its optional pin settings, consult the :adi:`AD8332` data sheet.
...
-  Open VisualAnalog on the connected PC. The appropriate part type should be listed in the status bar of the **VisualAnalog – New Canvas** window. Select the template that corresponds to the type of testing to be performed (see Figure 3, where the :adi:`AD9259` is shown as an example).\ |image3|\
...
-  To change features to settings other than the default settings, click the **Expand Display** button, located on the bottom right corner of the window (see Figure 4), to see what is shown in Figure 5.
...
-  Adjust the amplitude of the input signal so that the fundamental is at the desired level. Examine the **Fund Power** reading in the left panel of the **VisualAnalog Graph - AD9259 FFT** window (see Figure 11).\ |image11|\
...
\* Repeat this procedure for Channel B, Channel C, and Channel D. \* Click the disk icon within the* **VisualAnalog Graph - AD9259 FFT** *window to save the performance plot data as a .csv formatted file. See Figure 12 for an example.\

.. container:: centeralign

   *\ Figure 12. Typical FFT, :adi:`AD9259` //
...
.. |image1| image:: https://wiki.analog.com/_media/resources/eval/ad9259_set-up.jpg
   :width: 600px
.. |image2| image:: https://wiki.analog.com/_media/resources/eval/ad9259_eval_board_bandwidth.jpg
   :width: 400px
.. |image3| image:: https://wiki.analog.com/_media/resources/eval/ad9259_va_newcanvas_window.jpg
   :width: 400px
.. |image4| image:: https://wiki.analog.com/_media/resources/eval/ad9259_va_toolbar_collapsed.jpg
   :width: 400px
.. |image5| image:: https://wiki.analog.com/_media/resources/eval/ad9259_va_mainwindow_expanded.jpg
   :width: 400px
.. |image6| image:: https://wiki.analog.com/_media/resources/eval/ad9259_spicon_chip_id.jpg
   :width: 400px
.. |image7| image:: https://wiki.analog.com/_media/resources/eval/ad9259_spicon_newbutton.jpg
   :width: 400px
.. |image8| image:: https://wiki.analog.com/_media/resources/eval/ad9259_spicon_adcbase0_tab.jpg
   :width: 400px
.. |image9| image:: https://wiki.analog.com/_media/resources/eval/ad9259_spicon_ex_adc_a_tab.jpg
   :width: 400px
.. |image10| image:: https://wiki.analog.com/_media/resources/eval/ad9259_va_toolbar_run.jpg
   :width: 400px
.. |image11| image:: https://wiki.analog.com/_media/resources/eval/ad9259_va_graphwindow.jpg
   :width: 400px
