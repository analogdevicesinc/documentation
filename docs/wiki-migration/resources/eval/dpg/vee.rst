...
Connecting to VEE involves the same commands as used in other applications, using the .NET Operation Builder. Follow these steps to create a basic connection to a DPG and download data. For a complete function reference, see the `programming_reference <https://wiki.analog.com/programming_reference>`__ section.
