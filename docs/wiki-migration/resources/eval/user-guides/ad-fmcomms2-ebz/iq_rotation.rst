...
In many multi-channel RF systems like the `AD-FMCOMMS5 <https://wiki.analog.com/../ad-fmcomms5-ebz>`__, or even on the `AD-FMCOMMS2 <https://wiki.analog.com/../ad-fmcomms2-ebz>`__, `AD-FMCOMMS3 <https://wiki.analog.com/../ad-fmcomms3-ebz>`__ it is necessary to measure or correct for the phase difference between two complex (I/Q) RF signals.

From a pure mathematical description, a single sine wave has no phase, a phase can only develop between two different sine waves. The added complexity is that we do not have a single real signal (sine wave), we have a complex signal :math:`(I + Qi) = cos(\omega) + sin(\omega)i = e^{{i}{\omega}}` . Rotating this signal by 180° or :math:`\pi` radians, becomes :math:`e^{{i}{\pi}} + 1` , and this is the result we want to be able to find, and eventually correct for. If you are not sure why the math works this way, check out this `explanation <http://whiteboard.ping.se/SDR/IQ>`__.

In a previous section, we talked about `I/Q correction <https://wiki.analog.com/../ad-fmcomms1-ebz/iq_correction>`__ - phase shifting Q with respect to a fixed I, and making the magnitudes of I and Q equal. In the :adi:`AD9361`/:adi:`AD9364` this is done inside the device automatically, and is know as "Quadrature correction".
...
Although Simulink may have a block to directly measure `phase difference between two complex input signals <https://www.mathworks.com/help/comm/ref/complexphasedifference.html>`__, most other environments do not, and we will need to get a better understanding how to do this.
...
-  single point analysis : this uses the above picture, and looks at the phase between 2 points. Since we are not really interested in the a point in time (since this isn't how complex modulation works), this isn't applicable, or implemented. The below methods treat things with multiple captured samples, and are used, but are mostly based on this (`Law of cosines <https://en.wikipedia.org/wiki/Law_of_cosines>`__ developed by `Euclid of Alexandria <https://en.wikipedia.org/wiki/Euclid>`__ in the 3rd century BC). The only issue is that the law of cosines, always provides the inside, or obtuse angle, not the reflex one, which may be the one we are interested in.
...
So for a 1MHz baseband sine wave, that is off by .5°, that is a :math:`Deltat = 0.5{°} / 360{°} ~ \times ~ 1 \times 10^6Hz = 1.389 \times 10^{-9} seconds = 1.389ns` . Like all electromagnetic waves, `radio waves <https://en.wikipedia.org/wiki/Radio_wave>`__ travel at the `speed of light <https://en.wikipedia.org/wiki/Speed_of_light>`__ (~0.299792458 meter / ns). In this case 0.5°, means we could be ~41.64cm off any relative distance measurement (if that is what you are doing).
...
We can use the `trigonometric sum/difference identities <http://www.sosmath.com/trig/Trig5/trig5/trig5.html>`__ to re-write that as:
