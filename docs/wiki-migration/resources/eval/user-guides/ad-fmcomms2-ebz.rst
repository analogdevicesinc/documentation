...
The AD-FMComms2-EBZ board is very similar to the `ad-fmcomms3-ebz <https://wiki.analog.com/ad-fmcomms3-ebz>`__ board with only one exception, the RX/TX RF differential to single ended balun/transformer. The AD-FMComms3-EBZ is more targetted for wider tuning range applications, that is why we use the `TCM1-63AX+ <http://www.minicircuits.com/pdfs/TCM1-63AX+.pdf>`__ from mini Circuits as the RF transformer of choice. We affectionately call the FMCOMMS3-EBZ the "Software Engineers" platform, and the FMCOMMS2-EBZ, the "RF Engineers" platform to denote the difference.
...
-  `Modelling and Simulating Analog Devices’ RF Transceivers with MATLAB and RF Blockset (formerly SimRF) <https://www.mathworks.com/videos/modelling-and-simulating-analog-devices-rf-transceivers-with-matlab-and-simrf-89934.html>`__
