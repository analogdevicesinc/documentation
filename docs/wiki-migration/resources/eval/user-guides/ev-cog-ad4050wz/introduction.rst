...
-  An MCU *Cog* (such as the `EV-COG-AD4050WZ <https://wiki.analog.com/resources/eval/user-guides/eval-cog-ad4050wz/cog_hw_userguide>`__) which has an on-board debugger and test-points for monitoring energy consumption.
-  An optional connectivity *Cog* (such as the :doc:`EV-COG-BLEINTP1Z </wiki-migration/resources/eval/user-guides/eval-cog-ad3029lz/ev-cog-interposer>`) which offers BTLE, LPWAN or WiFi connectivity options.
-  An optional application specific add-on board (*Gear*) which might have an optimized sensor signal chain, specific to the use-case. For example - an add-on board targeted at smart agriculture use-cases might have Light, Temperature and Humidity Sensors. For initial prototyping, a specialized Gear is available which provides access to Arduino and PMOD connectors in addition to debug connectors - see :doc:`EV-GEAR-EXPANDER1Z </wiki-migration/resources/eval/user-guides/eval-cog-ad3029lz/ev-gear-expander>`.
...
| 
| End Document
| :doc:`Back </wiki-migration/resources/eval/user-guides/ev-cog-ad4050wz>`
