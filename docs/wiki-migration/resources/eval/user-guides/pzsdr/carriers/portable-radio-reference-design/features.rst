...
The external DC power supply input is a 5.5mm barrel jack, which works with the wide input voltage range of the `LTC3899 <http://www.linear.com/product/LTC3899>`__ (4.5 to 60V) allowing the radio to be powered from a multitude of external supplies including:
...
The system selects between three input power sources; Power over Ethernet (V1), DC Supply (V2), battery(V3). The `LTC4417 <http://www.linear.com/product/LTC4417>`__ Power Path Selector prioritizes the inputs as V1, V2, V3. If V1 is valid it is the input used to power the system (will call this V_PCB). If V1 invalid, V2 is used. If V2 invalid, V3 is used. If V3 invalid, then the system is off.
...
The `LTC4417 <http://www.linear.com/product/LTC4417>`__ validates each input by comparing it to an under voltage threshold (UV) and over voltage threshold (OV). The voltage must remain inside the window for 256 ms or it is considered invalid and the timer resets.
...
-  The `LTC4417 <http://www.linear.com/product/LTC4417>`__ connects 5V_EXT to the 5V rail and the transition is complete.
...
720ms... that is a long time! Why is this a problem?? The `LTC4417 <http://www.linear.com/product/LTC4417>`__ validation time is 100ms to 412ms. So, once the ADP1621 regulates the battery voltage to create '+5V_BATT', the LTC4417 can take anywhere from 100ms to 412ms to confirm this voltage is ready for use in the rest of the system. Unfortunately, 412ms is longer than 304ms, and this does not include additional constraints:
...
The `LTC2942 <http://www.linear.com/product/LTC2942>`__ measures battery voltage and chip temperature. A precision coulomb counter integrates current through a sense resistor between the battery’s positive terminal and the load or charger. Battery voltage and on-chip temperature are measured with an internal 14-bit No Latency ∆∑™ ADC. The three measured quantities (charge, voltage and temperature) are stored in internal registers accessible via the onboard SMBus/I2C interface.
...
-  The `Quectel L76-M33 <http://www.quectel.com/UploadFile/Product/Quectel_L76_GNSS_Specification_V1.3.pdf>`__ IC is a small (10.1mm x 9.7mm) GNSS module
   ...
   -  A `Dome Antenna <https://www.inventeksys.com/products-page/gps-antennas/antdom-05-01-wpm-gps-antenna/>`__ with 5 meter cable is used to receive the GPS signals, verify position and time.
   -

   |image5|

   -  `To verify the reading <https://mynasadata.larc.nasa.gov/latitudelongitude-finder/>`__ Analog Devices has an office at 831 Woburn Street, Wilmington MA 01887.
   -

   |image6|

   -  If you are unfamiliar with GPS, or simply need a refresher... `Global Positioning System Basics <https://en.wikipedia.org/wiki/Global_Positioning_System#Basic_concept_of_GPS>`__
...
-  The :adi:`ADAU1761 <en/products/processors-dsp/sigmadsp-audio-processors/adau1761.html#product-overview>` stereo audio codec coupled with an accessory detection switch, allows use of any OMTP or CTIA headphone standard.
   ...
   -  For more information regarding how the tip, ring, ring, sleeve standard works. `TRRS Audio Standards <https://en.wikipedia.org/wiki/Phone_connector_(audio)#TRRS_standards>`__
   -
...
-  The `DS3232 <https://www.maximintegrated.com/en/products/analog/real-time-clocks/DS3232.html>`__ is an I2C real-time clock (RTC) with battery-backed SRAM. If the system loses power, the RTC is powered from the battery voltage to it can keep time accurately at all times.
...
.. |image2| image:: https://wiki.analog.com/_media/resources/eval/user-guides/pzsdr/carriers/portable_radio_concept/radio_menu.jpg
   :width: 300px
.. |image3| image:: https://wiki.analog.com/_media/resources/eval/user-guides/pzsdr/carriers/portable_radio_concept/charge_menu.jpg
   :width: 300px
.. |image4| image:: https://wiki.analog.com/_media/resources/eval/user-guides/pzsdr/carriers/adrv1cc-box/imu.jpg
   :width: 300px
.. |Unit being tested to make sure nothing malfunctions, breaks or shakes loose| image:: https://wiki.analog.com/_media/resources/eval/user-guides/pzsdr/carriers/adrv1cc-box/imu_being_tested.jpg
   :width: 600px
.. |image5| image:: https://wiki.analog.com/_media/resources/eval/user-guides/pzsdr/carriers/adrv1cc-box/gps1.jpg
   :width: 600px
.. |image6| image:: https://wiki.analog.com/_media/resources/eval/user-guides/pzsdr/carriers/adrv1cc-box/gps3.jpg
   :width: 600px
.. |image7| image:: https://wiki.analog.com/_media/resources/eval/user-guides/pzsdr/carriers/packrf/audio_standards.jpg
   :width: 600px
