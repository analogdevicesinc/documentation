.. warning:: Conversion failed for ``resources/eval/user-guides/ade9430``

   Reason: pandoc error: Error at "/tmp/tmpea8qg6nl.txt" (line 284, column 1):
expecting inline
not found
\\
^
...
.. code-block:: text
   ...
   ====== ADE9430 Technical Reference Manual ======
   \\ 
   \\
   Functionality and Features of the ADE9430 High Performance, Polyphase Energy, and Class S Power Quality Monitoring IC
   \\
   \\
   {{  :resources:eval:user-guides:ade9430_trm:15210-001-mod.svg? 600 |}}
   <WRAP centeralign>**Figure 1 FUNCTIONAL BLOCK DIAGRAM**</WRAP>
   \\
   \\
   ====== Scope ======
   This reference manual provides a detailed description of the ADE9430 functionality and features. This document
   must be used in conjunction with the ADE9430 data sheet.
   
   ====== Overview ======
   The ADE9430 incorporates seven independent, second-order, Σ-Δ ADCs that sample simultaneously. Each ADC is
   24 bits and supports fully differential and pseudo differential inputs, which can go above and below ground. The
   ADE9430 includes a low noise, low drift, internal band gap reference. Set the EXT_REF bit in the CONFIG1 register
   if using an external voltage reference. Each ADC contains a programmable gain amplifier which allows a gain of 1, 2,
   or 4.
   ...
   The input signals on the IAP, IAN, IBP, IBN, ICP, ICN, VAP, VAN, VBP, VBN, VCP, and VCN pins must not exceed
   0.6V. The differential full-scale input range of the ADCs is ±1V peak (0.707V rms).
   Figure 2 and Figure 3 show two common types of input signals for an energy monitoring application. Figure 2 shows the maximum input allowed with differential antiphase signals. A current transformer with center tapped
   burden resistor generates differential, antiphase signals. Figure 3 shows the maximum input signal with pseudo
   differential signals, similar to those obtained when sensing the mains voltage signal through a resistive divider.
   \\
   \\
   ...
   \\
   \\
   Each ADC contains a programmable gain amplifier which allows a gain of 1, 2, or 4. The ADC produces full-scale
   output codes with an input of ±1V. With a gain of 1, this full-scale output corresponds to a differential antiphase
   input of 0.707V rms, as shown in Figure 2, full-scale output codes are produced with an input
   of 0.353V rms, as shown in Figure 3. At gain of 4, full-scale output codes are generated with a 0.1765V rms input
   signal. Note that the voltages on the xP and xN pins must be within ±0.6V, as specified in the data sheet.
   Write the x_GAIN bits in the PGA_GAIN register to configure the gain for each channel.
   
   {{ :resources:eval:user-guides:ade9430_trm:15523-002.svg? 500x500 |}}\\
   <WRAP centeralign>**Figure 2. Maximum Input Signal with Differential Antiphase Input with Common Mode Voltage = 0.1V, Gain = 1**</WRAP>
   \\
   {{ :resources:eval:user-guides:ade9430_trm:15523-003.svg? 500x500 |}}\\
   <WRAP leftalign>**Figure 3. Maximum Input Signal with Pseudo Differential Input with Common Mode Voltage = 0.1V, Gain = 2**</WRAP>
   \\
   \\
   
   ===== Interfacing to Current and Voltage Sensors =====
   Figure 4. Application Circuit with Current Transformer Current Sensor show the recommended circuits
   to connect to current transformer and Rogowski coil current sensor with external integrator. Figure 5. Application Circuit with Voltage Sensed Through Resistor Divider shows the interface circuit to measure the mains
   voltage.
   The anti-aliasing filter corner is chosen around 7 kHz to provide sufficient attenuation of out of band signals near
   the modulator clock frequency. The same RC filter corner is used on voltage channels as well, to avoid phase errors
   between current and voltage signals.
   \\
   \\
   {{ :resources:eval:user-guides:ade9430_trm:15523-004.svg? 500x200 |}}
   <WRAP centeralign>**Figure 4. Application Circuit with Current Transformer Current Sensor or Rogowski coil current sensor with external integrator**</WRAP>
   {{ :resources:eval:user-guides:ade9430_trm:15523-005.svg? 500x300 |}}
   <WRAP centeralign>**Figure 5. Application Circuit with Voltage Sensed Through Resistor Divider**</WRAP>
   \\
   \\
   
   
   ===== Internal RF Immunity Filter =====
   Energy metering applications require the meter to be immune to external radio frequency fields of 30 Volts per Meter or 30 (V/m), from
   80 MHz up to 10 GHz, according to IEC 61000-4-3. The ADE9430 has internal antialiasing filters to improve
   performance in this testing because it is difficult to filter these signals externally. The second-order, internal low-pass
   filter (LPF) has a corner frequency of 10 MHz. Note that external anti-alias filters are required to attenuate frequencies
   above 7 kHz, as shown in the Interfacing to [[resources:eval:user-guides:ADE9430#interfacing_to_current_and_voltage_sensors|Current and Voltage Sensors]] section.
   
   =====Modes of Operation =====
   Each ADC has two modes of operation: normal mode and disabled mode.
   In the normal mode of operation, ADCs are turned on and sample continuously. The CHNL_DIS register can be used
   to disable the ADCs individually.
   There are 2 different power modes available in the ADE9430 ([[resources:eval:user-guides:ADE9430#Power-On Sequence|see the Power Modes section]]). All ADCs
   are turned on in PSM0 power mode. In PSM3 mode, all ADCs are disabled and cannot be turned on.
   \\
   \\
   \\
   ...
   <WRAP center box 40%>
   <WRAP centeralign>**Table 1. PSM power modes**</WRAP>
   ...
   When a conversion has been completed, the DREADY bit of the STATUS0 register is set to 1. If the CF4_CFG[3:2]
   bits in the CONFIG1 register are equal to 11, the CF4/EVENT/DREADY pin corresponds to DREADY and pulses
   high to indicate when seven new ADC results are ready.
   In the ADE9430, the modulator sampling rate (MODCLK) is fixed at 2.048 MHz (CLKIN/12 = 24.576/12). The
   output data rate of the sinc filter is MODCLK/64, whereas the low-pass filter/decimator stage yields an output rate 4
   times slower than the sinc filter output rate. Figure 6 shows the digital
   filtering that takes the 2.048 MHz ADC samples and creates waveform information at a decimated rate of 32 kHz or 8
   kHz.
   \\
   \\
   {{ :resources:eval:user-guides:ade9430_trm:data_path_following_adc_stage.svg? 600x400 |}}
   <WRAP centeralign>**Figure 6. Datapath Following ADC Stage**</WRAP>
   \\
   \\
   
   
   <WRAP center box 55%>
   <WRAP centeralign>**Table 2. Output Data Rates**</WRAP>
   ...
   \\
   \\
   The ADC data in the waveform buffer is stored as 32-bit data by shifting left by 4 bits and sign extending, as shown
   in Table 3.
   \\
   \\
   <WRAP center box 40%>
   <WRAP centeralign>**Table 3. 32-Bit ADC Data Format**</WRAP>
   ^Bits[31:28] ^ Bits[27:4] ^ Bits[3:0]^
   ^SE|ADC_DATA[23:0]|0000|
   </WRAP>
   \\
   \\
   The expected output code in the waveform buffer from the sinc filter when input is at 1 V peak is 67,107,786
   (decimal). The expected output code in the waveform buffer from the decimator filter when input is at 1 V peak is
   74,518,668. See the Waveform Buffer section for more information.
   \\
   \\
   ====== Voltage Reference ======
   The ADE9430 supports a 1.25 V internal reference. An external reference can be connected between the REFIN and
   REFGND pins. When using an external voltage reference, set the EXT_REF bit of the CONFIG1 register, which
   disables the internal reference buffer.
   
   ====== Crystal Oscillator/External Clock ======
   The ADE9430 contains a crystal oscillator. Alternatively, a digital clock signal can be applied at the CLKIN pin of the
   ADE9430.
   When a crystal is used as the clock source for the ADE9430, attach the crystal and the ceramic capacitors, with
   capacitances of CL1 and CL2, as shown in Figure 7. It is not recommended
   to attach an external feedback resistor in parallel to the crystal.
   When a digital clock signal is applied at the CLKIN pin, the inverted output is available at the CLKOUT pin. This
   output is not buffered internally and cannot be used to drive any other external devices directly. Note that CLKOUT is
   available in PSM0 operating mode only.
   
   {{ :resources:eval:user-guides:ade9430_trm:crystal_application_circuit.svg? 400x400 |}}
   <WRAP centeralign>**Figure 7. Crystal Application Circuit**</WRAP>
   \\
   \\
   \\
   \\
   ===== Crystal Selection =====
   The transconductance of the crystal oscillator circuit in the ADE9430, gm, is provided in the data sheet. It is
   recommended to have 3 to 5 times more gm than the calculated gm<sub>CRITICAL</sub> for the crystal.
   The following equation shows how to calculate the gm<sub>CRITICAL</sub> for the crystal from information given in the crystal
   data sheet:
   
   gm<sub>CRITICAL</sub> = 4 × ESR<sub>MAX</sub> × 1000 × (2 × π × f<sub>CLK(Hz)</sub>)<sup>2</sup> × (C<sub>0</sub> + C<sub>L</sub>)<sup>2</sup>
   
   where: 
   gm<sub>CRITICAL</sub> is the minimum gain required to start the crystal, expressed in mA/V. 
   ESR<sub>MAX</sub> is the maximum ESR, expressed in ohms. 
   fCLK is 24.576 MHz, expressed in Hz as 24.576 × 10<sup>6</sup>. 
   C<sub>0</sub> is the maximum shunt capacitance, expressed in farads. 
   C<sub>L</sub> is the load capacitance, expressed in farads.
   
   Crystals with low ESR and smaller load capacitance have a lower gm<sub>CRITICAL</sub> and are easier to drive.
   The ADE9430 evaluation board uses a crystal manufactured by Abracon (Part Number ABLS-24.576MHZ-8-L4Q-FT), which has a maximum ESR of 40 Ω, load capacitance of 8 pF, and maximum shunt capacitance of 7 pF, which
   results in a gm<sub>CRITICAL</sub> of 0.86 mA/V:
   
   gm<sub>CRITICAL</sub> = 4 × ESR<sub>MAX</sub> × 1000 × (2 × π × f<sub>CLK(Hz)</sub>)<sup>2</sup> × (C<sub>0</sub> + C<sub>L</sub>)<sup>2</sup>
   
   gm<sub>CRITICAL</sub> = 4 × 40 × 1000 × (2 × π × 24.576 × 10<sup>6</sup>)<sup>2</sup> × (7 × 10<sup>−12</sup> + 8 × 10<sup>−12</sup>)<sup>2</sup> = 0.86
   
   The gain of the crystal oscillator circuit in the ADE9430, gm, provided in the data sheet is more than 5 times
   gm<sub>CRITICAL</sub>; therefore, there is sufficient margin to start up this crystal.
   ...
   • CP<sub>1</sub> and CP<sub>2</sub> : parasitic capacitances on the clock pins formed due to printed circuit board
   (PCB) traces.
   
   • C<sub>in1</sub> and C<sub>in2</sub> : internal capacitances of the CLKIN and CLKOUT pins respectively.
   
   • CL<sub>1</sub> and CL<sub>2</sub> : selected load capacitors to get the correct combined CL for the crystal.
   
   
   The internal pin capacitances, C<sub>in1</sub> and C<sub>in2</sub>, are 4 pF each, as shown in the data sheet. To find the values of CP<sub>1</sub>
   and CP<sub>2</sub>, measure the capacitance on each of the clock pins of the PCB, CLKIN and CLKOUT, respectively, with respect to the AGND pin. If the measurement is done after soldering the IC to the PCB, subtract out the 4 pF internal capacitance of the clock pins to find the actual value of parasitic capacitance on each of the crystal pins.
   
   To select the appropriate capacitance value for the ceramic capacitors, calculate CL1 and CL2 from the following equation:
   
   CL = [(CL<sub>1</sub> + CP<sub>1</sub> + C<sub>IN1</sub>) × (CL<sub>2</sub> + CP<sub>2</sub> + CIN<sub>2</sub>)] / (CL<sub>1</sub> + CP<sub>1</sub> + CIN<sub>1</sub> + CL<sub>2</sub> + CP<sub>2</sub> + C<sub>IN2</sub>) (1)
   
   Select CL1 and CL2 such that the total capacitance on each clock pins is
   
   CL<sub>1</sub> + CP<sub>1</sub> + C<sub>IN1</sub> = CL<sub>2</sub> + CP<sub>2</sub> + C<sub>IN2</sub> (2)
   
   Using Equation 1 and Equation 2, the values of CL<sub>1</sub> and CL<sub>2</sub> can be calculated.
   ...
   \\
   CP<sub>1</sub> = CP<sub>2</sub> = 2 pF, Equation 1 implies that
   
   CL = [(CL<sub>1</sub> + CP<sub>1</sub> + C<sub>IN1</sub>) × (CL<sub>2</sub> + CP<sub>2</sub> + C<sub>IN2</sub>)]/(CL<sub>1</sub> + CP<sub>1</sub> + C<sub>IN1</sub> + CL<sub>2</sub> + CP<sub>2</sub> + C<sub>IN2</sub>)
   
   8 pF = [(CL<sub>1</sub> + 2 pF + 4 pF) × (CL<sub>2</sub> + 2 pF + 4 pF)]/(CL<sub>1</sub> + 2 pF + 4 pF + CL<sub>2</sub> + 2 pF + 4 pF)
   
   Assuming that CL<sub>1</sub> = CL<sub>2</sub>, to satisfy Equation 2,
   
   8 pF = [(CL<sub>1</sub> + 6 pF) × (CL<sub>1</sub> + 6 pF)]/(CL<sub>1</sub> + 6 pF + CL<sub>1</sub> + 6 pF)
   
   8 pF = [(CL<sub>1</sub> + 6 pF) × (CL<sub>1</sub> + 6 pF)]/[2 × (CL<sub>1</sub> + 6 pF)]
   
   8 pF = (CL<sub>1</sub> + 6 pF)/2
   
   Therefore, CL<sub>1</sub> = CL<sub>2</sub> = 10 pF.
   
   Based on this example, 10 pF ceramic capacitors are selected for CL<sub>1</sub> and CL<sub>2</sub>.
   
   ====== Power Management ======
   
   The ADE9430 offers two operating modes, PSM0 and PSM3. The entry into the power modes is controlled by the
   PM1 and PM0 pins. These pins are checked continuously to determine which operating mode to enter. If it is desired
   to place the ADE9430 into a low power reset state, PSM3 can be used.
   
   ===== Power-On Sequence =====
   
   After power is applied to the VDD pin of the ADE9430 IC, the device checks the state of the PM0 and PM1 pins
   to check the power supply mode. If in PSM0 mode
   (PM1 and PM0 = 00 or 01, 00 recommended) and the RESET pin is high, the AVDD and DVDD low dropout
   regulators (LDOs) are turned on when VDD reaches 2.4 V to 2.6 V. If the RESET pin is low, the AVDD and DVDD
   LDOs are not turned on. Note that there is a clamp that limits the current used to charge the AVDD and DVDD LDOs
   to 17 mA per LDO.
   \\
   \\
   When AVDD and DVDD are both above 1.3 V to 1.5 V and VDD is above 2.4 V to 2.6 V, a 20 ms timer is started to
   allow additional time for the supplies to come to their normal potentials (VDD between 2.97 V and 3.6 V, AVDD at
   1.9 V, and DVDD at 1.7 V). After this timer has elapsed, the crystal oscillator is started.
   The RSTDONE interrupt is triggered 26 ms later, bringing the IRQ1 pin low and setting the RSTDONE bit in the
   STATUS1 register.
   \\
   \\
   The RSTDONE bit being set indicates to the user that the ADE9430 has finished its power-up
   sequence. The user can now configure the IC via the serial peripheral interface (SPI). After configuring the device,
   write the RUN register to start the DSP so that it starts making measurements. Note that registers from Address 0x000
   through Address 0x0FF and Address 0x400 through Address 0x5FF are restored to their default values during power on. Registers from Address 0x200 through Address 0x3FF are cleared within 500 µs from when the RUN register
   value changes from 0x0000 to 0x0001. Also note that the waveform buffer, Address 0x800 through Address 0xFFF, is
   not cleared after reset.
   \\
   \\
   In PSM3 mode, the AVDD and DVDD LDOs are not turned on. The RSTDONE interrupt does not occur, and the SPI
   port is not available
   \\
   \\
   
   {{ :resources:eval:user-guides:ade9430_trm:ade9430_power_on_sequence_for_psm0.svg?700x400 |}}
   <WRAP centeralign>**Figure 8. ADE9430 Power-On Sequence for PSM0**</WRAP>
   \\
   \\
   \\
   \\
   ...
   <WRAP centeralign>**Table 4. Power Modes (PSM0 and PSM3)**</WRAP>
   
   ^ PSMx Power Mode ^ Description ^ PM1 Pin ^ PM0 Pin ^ Functions Available ^ SPI Available? ^
   | PSM0 | Normal mode | 0 |<WRAP> 0 or 1,
   \\
   0 recommended </WRAP>| All functions | Yes |
   ...
   Power-on reset (POR) circuits monitor the VDD, AVDD, and DVDD supplies. If AVDD or DVDD drops below 1.3 V
   to 1.5 V, or VDD drops below 2.4 V to 2.6 V, the IC is held in reset and the power-on sequence begins again, waiting
   until AVDD and DVDD are above 1.3 V to 1.5 V and VDD is above 2.4 V to 2.6 V before starting the 20 ms POR
   timer. A RSTDONE interrupt on IRQ1 indicates when the ADE9430 can be reinitialized via the SPI.
   
   ===== Reset =====
   
   If the RESET pin goes low for 1 μs or the SWRST bit is set in the CONFIG1 register to initiate a software reset,
   the AVDD and DVDD LDOs are turned off. The power-on sequence resumes from the point where the AVDD and
   DVDD LDOs are turned on ([[resources:eval:user-guides:ADE9430#Power-On Sequence|see the Power-On Sequence for details]]). For applications that require
   putting the ADE9430 into a low power reset state, it is recommended to use PSM3, which consumes roughly 2 μA,
   instead of holding the IC in reset with the RESET pin low, which consumes 100 μA (see the data sheet for the exact
   current consumption).
   
   ===== Changing Power Modes =====
   
   The state of the PM1 and PM0 pins is continuously monitored. If the power mode changes from PSM0 to PSM3
   (PM1 and PM0 = 11) for 1 μs, the AVDD and DVDD LDOs are turned off. When the power mode switches back to
   PSM0, the power-on sequence resumes from the point where the AVDD and DVDD LDOs are turned on.
   ...
   <WRAP center box 100%>
   <WRAP centeralign>**Table 5. indicates the registers that hold current channel measurements and the rate at which they update.**</WRAP>
   ...
   |	ISUMRMS	|  Filtered rms of vector sum (AI_PCF + BI_PCF + CI_PCF ± NI_PCF); [[resources:eval:user-guides:ADE9430#Neutral Current RMS and Vector Current Sum|see the Neutral Current RMS and Vector Current Sum section]]  |  fDSP = 8 ksps    |
   |	IPEAK	|	Peak current channel sample; [[resources:eval:user-guides:ADE9430#Peak Detection|see the Peak Detection section]]	|	fDSP = 8 ksps	|
   |	ANGLx_xxx	|	Voltage to current or current to current phase angle; [[resources:eval:user-guides:ADE9430#Angle Measurement|see the Angle Measurement section]]	|	CLKIN/24 = 1024 ksps	|
   ...
   {{ :resources:eval:user-guides:ade9430_trm:adc_redirect_modulator_to_digital_datapath_multiplexing.svg? 700 x 700  }}
   <WRAP centeralign>**Figure 9. ADC_REDIRECT Modulator to Digital Datapath Multiplexing**</WRAP>
   \\
   \\
   ...
   xIGAIN = ROUND[(−1 − 1) × 2<sup>27</sup>] = −268435456 = 0xF000_0000
   
   If the multipoint phase and gain feature is used, it is recommended to use the xIGAIN for the primary correction, done at the nominal current for the meter ([[resources:eval:user-guides:ADE9430#Multipoint Phase/Gain Calibration|see the Multipoint Phase/Gain Calibration section for more information]]).
   
   Note that for a given phase,
   
   <nowiki>|Current Channel Gain × Voltage Channel Gain × Power Gain| ≤ 3.75 </nowiki>
   
   
   
   ===== IB Calculation Using ICONSEL =====
   
   Write the ICONSEL bit in ACCMODE to calculate IB = −IA − IC. This setting can help save the cost of a current transformer in some 3-wire delta configurations. [[resources:eval:user-guides:ADE9430#Applying the ADE9430 to a 3-Wire Delta Service|See the Applying the ADE9430 to a 3-Wire Delta Service section]] for more information.
   ...
   The ADE9430 high-pass filter on the current and voltage channels is enabled by default. It can be disabled by writing the HPFDIS bit in the CONFIG0 register equal to 1.
   ...
   <WRAP center box 80%>
   <WRAP centeralign>**Table 6. HPF Corner Gain with 50 Hz Input Signal**</WRAP>
   ...
   Voltage Channel Delay° = <m>{f_LINE / f_DSP } * 360^circ{}</m>
   
   \\
   Voltage Channel Delay° = <m>{50 /  8000} * 360^circ = 2.25^circ</m>
   
   
   
   
   The current channel is then delayed by a digital filter, according to the value programmed into the xPHCALx register. 
   The resulting phase correction depends on the value in the xPHCALx register. The following equation provides the phase correction between the input current and voltage after the combined voltage and current delays. 
   In this formula, PhaseCorrection° is positive to correct a current that lags the voltage, and PhaseCorrection° is negative to correct a situation where the current leads the voltage, such as occurs with a current transformer:
   \\
   \\
   Phase Correction° = <m>arctan( {-sin omega{}} / {xPHCALx * 2^-27 + cos omega{}}) - arctan ({-xPHCALx * 2^-27 + sin omega{}} / {1 + xPHCALx * 2^-27 * cos omega{}}) - omega{}</m>
   \\
   \\
   where ω = 2 × π × f<sub>LINE</sub> / f<sub>DSP</sub>.
   
   The xPHCALx register value can be calculated from the desired phase correction according to this equation:
   \\
   \\
   xPHCALx = <m>({ {sin(phi - omega) + sin omega} / {sin(2 * omega - phi )}}) * 2^27</m>
   \\
   \\
   
   For example, if f<sub>LINE</sub> = 50 Hz, f<sub>DSP</sub> = 8 kHz, and the current leads the voltage by 0.1 degrees, Phase Correction° = −0.1°. Write xPHCALx = 0xFFD3_7760 to correct for this.
   
   ω = 2 × π × 50/8000 = 0.03927
   \\
   \\
   xPHCALx = <m>({ {sin(RADIANS(-0.1) - 0.03927) + sin 0.03927} / {sin(2 * 0.03927 - RADIANS(-0.1 ))}}) * 2^27</m>=
   − 2918553 = 0xFFD3_7767
   \\
   \\
   {{ :resources:eval:user-guides:ade9430_trm:phase_compensation_example_for_current_transformer_where_the_current_leads_the_voltage.svg? 600 x 600 |}}
   <WRAP centeralign>**Figure 10. Phase Compensation Example for Current Transformer, where the Current Leads the Voltage**</WRAP>
   \\
   {{ :resources:eval:user-guides:ade9430_trm:phase_compensation_example_where_current_lags_voltage.svg? 600 x 600 |}}
   <WRAP centeralign>**Figure 11. Phase Compensation Example where Current Lags Voltage**</WRAP>
   \\
   ...
   \\
   \\
   Voltage Channel Delay° = <m>({60/8000} * 360^circ) = 2.7^circ</m>
   ...
   \\
   {{ :resources:eval:user-guides:ade9430_trm:ade9000_multipointcalibration.svg? 700 x 700 |}}
   <WRAP centeralign>**Figure 12. Multipoint Phase and Gain Calibration**</WRAP>
   \\
   \\
   ...
   <WRAP center box 100%>
   <WRAP centeralign>**Table 7. Voltage Channel Measurement Update Rates**</WRAP>
   ...
   |	<nowiki>AV_LPF__DAT</nowiki>	|	VA sinc4 + IIR LPF filter output	|	fDSP = 8 ksps	|
   |	<nowiki>BV_LPF__DAT</nowiki>	|	VB sinc4 + IIR LPF filter output	|	fDSP = 8 ksps	|
   |	<nowiki>CV_LPF__DAT</nowiki>	|	VC sinc4 + IIR LPF filter output	|	fDSP = 8 ksps	|
   ...
   |	VPEAK	|	Peak current channel sample; see the [[resources:eval:user-guides:ADE9430#Peak Detection|Peak Detection section]]	|	fDSP = 8 ksps	|
   ...
   |	ANGLx_xxx	|  Voltage to current or current to current phase angle; see the [[resources:eval:user-guides:ADE9430#Angle Measurement|Angle Measurement section]]  |	CLKIN/24 = 1024 ksps	|
   </WRAP>
   \\
   \\
   ...
   The xVGAIN registers can be used to calibrate the voltage channel of each phase. The xVGAIN register has the same scaling as the xIGAIN register. See the [[resources:eval:user-guides:ADE9430#Current Channel Gain, xIGAIN|Current Channel Gain, xIGAIN section]] for the equation.
   ...
   <WRAP center box 45%>
   <WRAP centeralign>**Table 8. Full-Scale ADC Codes**</WRAP>
   ...
   \\
   \\
   <m>xxRMS = sqrt{{xxRMS_0}^2 + (2^15 * xxRMSOS)}</m>
   \\
   \\
   
   where xxRMS<sub>0</sub> is the initial xRMS register value before offset calibration.
   ...
   \\
   \\
   <m>AIRMSOS = {52702^2 - 53280^2}/2^15 =</m>  -1869d =0xFFFF8B8
   
   Table 9 shows the rms settling time to 99% of full scale for a 50 Hz signal.
   
   <WRAP center box 55%>
   <WRAP centeralign>**Table 9. RMS Settling Time**</WRAP>
   ...
   <WRAP center box 63%>
   <WRAP centeralign>**Table 10. ISUM Configuration Options**</WRAP>
   ...
   \\
   \\
   <m>ISUMLVL = ({xIRMS FULL SCALE}/X)</m>
   \\
   \\
   ...
   {{ :resources:eval:user-guides:ade9430_trm:instantaneous_active_power_and_low-pass_filtered_active_power_at_a_power_factor_of_1.svg? 600 x 600 | }}
   <WRAP centeralign>**Figure 13. Instantaneous Active Power and Low-Pass Filtered Active Power at a Power Factor of 1**</WRAP>
   \\
   \\
   ...
   xWATTOS = xWATT<sub>EXPECTED</sub> − xWATT<sub>MEASURED</sub>
   ...
   \\
   \\
   <m>xWATTOS = 1 / {20694066/10000} = 0.05%</m>
   \\
   \\
   ...
   \\
   <WRAP center box 57%>
   <WRAP centeralign>**Table 11. Total Active Power Settling Time**</WRAP>
   ...
   The total reactive power offset can be calibrated for even better performance over a wide dynamic range using the xVAROS register. xVAROS has the same scaling as xVAR; see the [[resources:eval:user-guides:ADE9430#Total Active Power|Total Active Power section]] to understand how to calculate this register value. Table 12 shows the settling times for total reactive power for a 50 Hz signal.
   
   <WRAP center box 60%>
   <WRAP centeralign>**Table 12. Total Reactive Power Settling Time**</WRAP>
   ...
   Apparent power is generated by multiplying the current rms measurement, xIRMS, by the corresponding voltage rms, xVRMS, and then applying a gain correction, xPGAIN. The result is stored in the xVA register. Note that the offset of the total apparent power calculation is performed by calibrating the xIRMS and xVRMS measurements, using the xIRMSOS and xVRMSOS registers; [[resources:eval:user-guides:ADE9430#Filter-Based Total RMS|see the Filter-Based Total RMS section]] for more information on the rms calculation.
   ...
   <WRAP center box 69%>
   <WRAP centeralign>**Table 13. Total Apparent Power Settling Time**</WRAP>
   ...
   The ADE9430 offers fundamental current and voltage rms measurements using the proprietary fundamental estimation technique described in the [[resources:eval:user-guides:ADE9430#Fundamental Measurements|Fundamental Measurements section]]. The xIFRMSOS and xVFRMSOS registers allow the offset to be calibrated for even better performance at low input signal levels.
   ...
   \\
   \\
   <m>xxFRMS = sqrt{{xxFRMS_0}^2 +2^15 * xxFRMSOS}</m>
   \\
   \\
   ...
   {{ :resources:eval:user-guides:ade9430_trm:fundamental_watt_afwatt_calculation.svg? 550 x 550 |}}
   <WRAP centeralign>**Figure 14. Fundamental WATT, AFWATT Calculation**</WRAP>
   \\
   \\
   xFWATTOS has the same scaling as xFWATT; [[resources:eval:user-guides:ADE9430#Total Active Power|see the Total Active Power section]] to understand how to calculate this register value.
   ...
   {{ :resources:eval:user-guides:ade9430_trm:fundamental_reactive_power_afvar.svg? 600 x 600 |}}
   <WRAP centeralign>**Figure 15. Fundamental Reactive Power, AFVAR**</WRAP>
   \\
   \\
   ...
   xFVAROS has the same scaling as xFVAR; see the [[resources:eval:user-guides:ADE9430#Total Active Power|Total Active Power section]] to understand how to calculate this register value.
   \\
   \\
   ...
   The ADE9430 offers fundamental rms measurements using the proprietary fundamental estimation technique described in the [[resources:eval:user-guides:ADE9430#Fundamental Measurements|Fundamental Measurements]] and [[resources:eval:user-guides:ADE9430#Fundamental RMS|Fundamental RMS sections]]. The fundamental rms measurements, xIFRMS and xVFRMS, are multiplied together to obtain fundamental apparent power. This is then gained by xPGAIN and stored in the xFVA register. Figure 16 shows the signal chain for the AFVA measurement.
   
   Note that offset correction can be performed by calibrating the AIFRMS and AVFRMS measurements.
   
   
   {{ :resources:eval:user-guides:ade9430_trm:fundamental_apparent_power_afva.svg? 500 x 500 |}}
   <WRAP centeralign>**Figure 16. Fundamental Apparent Power, AFVA**</WRAP>
   \\
   \\
   ...
   {{ :resources:eval:user-guides:ade9430_trm:per_phase_power_and_energy_calculations_from_xi_pcf_and_xv_pcf_waveforms.svg? 600 x 600 |}}
   <WRAP centeralign>**Figure 17. Per Phase Power and Energy Calculations from xI_PCF and xV_PCF Waveforms**</WRAP>
   \\
   \\
   ...
   Energy measurements update every f<sub>DSP</sub> = 8 ksps by default and can store up to 106 sec of accumulation at full scale. Alternatively, these registers can be set into a different accumulation mode where they update after a user defined number of line cycles or samples.
   ...
   <WRAP centeralign>**Table 14. Watt Related Register Update Rates**</WRAP>
   ...
   \\
   {{ :resources:eval:user-guides:ade9430_trm:awatt_accumulation_into_energy_and_power_using_no-load_threshold_and_signed_accumulation_mode.svg? 800 x 800 |}}
   <WRAP centeralign>**Figure 18. AWATT Accumulation into Energy and Power, Using No-load Threshold and Signed Accumulation Mode**</WRAP>
   \\
   Figure 18 shows how AWATT is accumulated into the AWATTHR and AWATT_ACC registers. 
   A no-load threshold is applied and the energy is checked to determine whether to accumulate the AWATT sample into the internal energy accumulator. 
   The internal energy accumulator is either added to the AWATTHR register or overwrites it at a EGYRDY rate. 
   Set the EGY_PWR_EN bit in EP_CFG register to run the energy and power accumulator.
   \\
   \\
   ...
   <WRAP centeralign>**Table 15. No-Load Condition Evaluation Time**</WRAP>
   ...
   \\
   \\
   xNL_LVL = <m>({xWATT FULL SCALE * 64} / X)</m>
   \\
   \\
   ...
   \\
   \\
   xNL_LVL =<m>({20694066d * 64} / {50000d})=26488d =</m>0x6778
   \\
   \\
   When a phase is in no-load, every f<sub>DSP</sub> = 8 ksps, zero energy is accumulated into the energy registers and CF accumulation.
   
   Note that the x_ACC registers are not affected by no-load detection. Even when in no-load, any power calculated in the respective xWATT, xVAR, and xVA registers is accumulated into the corresponding x_ACC register every f<sub>DSP</sub> = 8 ksps.
   ...
   There is also an option to indicate the no-load status on the EVENT pin; [[resources:eval:user-guides:ADE9430#Interrupts/EVENT|see the Interrupts/EVENTEVENT section]] for more information.
   ...
   {{ :resources:eval:user-guides:ade9430_trm:no-load_detection_and_indication.svg? 600 x 600 |}}
   <WRAP centeralign>**Figure 19. No-Load Detection and Indication**</WRAP>
   \\
   \\
   ...
   \\
   \\
   Maximum Internal Energy Accumulator Time (sec) = <m>(2^41 / {20694066 * 8000 })</m> = 13.3 sec
   \\
   \\
   ...
   The EGYRDY update rate occurs after EGY_TIME + 1 f<sub>DSP</sub> samples or EGY_TIME + 1 half line cycles, according to the EGY_TMR_MODE bit in the EP_CFG register.
   ...
   \\
   \\
   Internal Energy Accumulation Time (sec) = <m>({EGY TIME + 1}/f_DSP)</m>
   \\
   \\
   ...
   \\
   \\
   Internal Energy Accumulation Time (sec) = <m>({8191 + 1}/ 8000)</m> = 1.024 sec
   \\
   \\
   ...
   \\
   \\
   Internal Energy Accumulation Time (sec) = <m>({EGY TIME + 1}/ {ZX Rate})</m>
   \\
   \\
   ...
   \\
   \\
   Internal Energy Accumulation Time (sec) = <m>({8191 + 1}/ 100)</m> = 81.92 sec
   \\
   \\
   ...
   {{ :resources:eval:user-guides:ade9430_trm:egyrdy_and_pwrrdy_update_rate.svg? 500 x500 |}}
   <WRAP centeralign>**Figure 20. EGYRDY and PWRRDY Update Rate**</WRAP>
   \\
   \\
   ...
   The energy registers in the ADE9430 are signed and 45 bits wide, split between two 32-bit registers, as shown in Figure 21. These accumulators update at a rate according to EGYRDY, as described in [[resources:eval:user-guides:ADE9430#User Energy Register Update Rate, EGYRDY|User Energy Register Update Rate, EGYRDY]]. The following equation shows how to calculate the time until the user accessible accumulator overflows with full-scale inputs and all digital gain and offset factors at zero, where AWATT_AT_FULL_SCALE refers to the nominal AWATT value with full-scale inputs. For this example, assume that the internal energy register is updating at every f<sub>DSP</sub> = 8 ksps sample.
   \\
   \\
   Maximum Internal Energy Accumulator Time (sec) = <m>(2^44 / { AWATT AT FULL SCALE * f_DSP})</m>
   \\
   \\
   ...
   \\
   \\
   Maximum Internal Energy Accumulator Time (sec) = <m>(2^44 / { 20694066 * 8000})</m>= 106.3 sec
   ...
   {{ :resources:eval:user-guides:ade9430_trm:internal_energy_register_to_awatthr_hi_and_awatthr_lo.svg? 500 x 500 |}}
   <WRAP centeralign>**Figure 21. Internal Energy Register to AWATTHR_HI and AWATTHR_LO**</WRAP>
   \\
   \\
   ...
   AWATTHR_HI = ROUNDDOWN(USER_ENERGY_ACCUMULATION × 2<sup>−13</sup>)
   ...
   \\
   \\
   CF(Hz)=<m>(f_DTOF * AWATT / xTHR * 512 * CFxDEN)</m>
   \\
   \\
   ...
   \\
   \\
   MAXIMUM_CF(Hz)=<m>(4.096 * 10^6 * 20694066) / (00100000h * 512 * 2)</m>
   
   where: f<sub>DTOF</sub> is 4.096MHz. AWATT is the value at full scale, 20694066. xTHR is 0x0010_0000. CFDEN is 2.
   
   The default CF pulse output using power-on reset values of xTHR and CFxDEN with full-scale inputs is 
   \\
   MAXIMUM CF (Hz) = <m>(4.096 * 10^6* 20694066)/(0000FFFFh * 512 * FFFFh)</m> = 38.5Hz
   ...
   \\
   \\
   <WRAP centeralign>**Table 16. CF Active Low Pulse Width and Duty Cycle based on CFx_LT and CF_LTMR**</WRAP>
   ...
   \\
   <m>Internal Power AccumulationTime(sec) = {(PWR TIME + 1)}/{8000}</m>
   \\
   \\
   The PWR_TIME[12:0] register allows up to (8191 + 1) = 8192 samples to be accumulated, which corresponds to 8192/8000 = 1.024 sec:
   \\
   \\
   <m>Internal Power AccumulationTime(sec) = ({8191 + 1}/{8000}) = 1.024</m> sec:
   
   The internal power accumulator overflows at the same rate as the internal energy accumulator (see the [[resources:eval:user-guides:ADE9430#Internal Energy Register Overflow Rate|Internal Energy Register Overflow Rate]] section).
   ...
   {{ :resources:eval:user-guides:ade9430_trm:internal_power_register_to_awatt_acc.svg? 500 x 500 |}}
   <WRAP centeralign>**Figure 22. Internal Power Register to AWATT_ACC**</WRAP>
   ...
   Power accumulation register = ROUNDDOWN(USER_POWER_ACCUMULATION × 2−<sup>13</sup>)
   ...
   AWATT_ACC = ROUNDDOWN(82776264000 × 2<sup>−13</sup>) = 10104524 = 0x009A_2ECC
   ...
   The power signs are updated at the same time as the xWATT_ACC, xFWATT_ACC, xVAR_ACC, and xFVAR_ACC registers and correspond to the sign of these registers. Note that the power registers and signs are updated after the number of f<sub>DSP</sub> = 8 ksps samples configured in the PWR_TIME register have elapsed, from 250 µs to 1.024 sec. The power sign change indication in the REVxPx bits are updated at the same time; [[resources:eval:user-guides:ADE9430#Power Accumulation|see the Power Accumulation Details section]] for more information.
   ...
   \\
   {{ :resources:eval:user-guides:ade9430_trm:power_accumulation_and_power_sign.svg? 500 x 500 |}}
   <WRAP centeralign>**Figure 23. Power Accumulation and Power Sign**</WRAP>
   \\
   \\
   ...
   \\
   {{ :resources:eval:user-guides:ade9430_trm:zero_crossing_detection_on_voltage_and_current_channels.svg? 500 x 500 |}}
   <WRAP centeralign>**Figure 24. Zero-Crossing Detection on Voltage and Current Channels**</WRAP>
   \\
   \\
   ...
   \\
   \\
   <m>ZXTHRSH ={(V PCF at Full Scale) * (LPF1 ATTENUATION)}/{X * 32 * 2^8}</m>
   \\
   \\
   ...
   \\
   \\
   <m>ZXTHRSH={(74,532,013) * (0.86)}/{100 * 32 * 2^8} = 78d</m>
   \\
   \\
   ...
   The input to the zero-crossing detection is (VA + VB − VC)/2 with the signal chain corresponding to Figure 25. As described in the [[resources:eval:user-guides:ADE9430#Applying the ADE9430 to Different Metering Configurations|Applying the ADE9430 to Different Metering Configurations section]], the ADE9430 can be used to meter different polyphase configurations. The VCONSEL[2:0] bits in the ACCMODE register are used to indicate this selection. If VCONSEL[2:0] is not equal to 0, the VB component in the combined zero-crossing circuit is set to zero.
   
   The same precautions are used to prevent noise from generating zero-crossing interrupts on this output. As described in the [[resources:eval:user-guides:ADE9430#Zero-Crossing Detection|Zero-Crossing Detection section]], signals below the ZXTHRSH threshold do not generate ZXCOMB outputs, and a minimum of 1 ms is required between ZXCOMB generation.
   \\
   {{ :resources:eval:user-guides:ade9430_trm:combined_zero_crossing_detection.svg? 800 x 800 |}}
   <WRAP centeralign>**Figure 25. Combined Zero-Crossing Detection**</WRAP>
   \\
   \\
   ...
   \\
   {{ :resources:eval:user-guides:ade9430_trm:zero-crossing_output_rates.svg? 800 x 800 |}}
   <WRAP centeralign>**Figure 26. Zero-Crossing Output Rates**</WRAP>
   \\
   \\
   
   <WRAP centeralign>**Table 17. Zero-Crossing Use in Other Functions**</WRAP>
   ...
   \\
   The ADE9430 line period measurement is done by taking the values low-pass filtered by LPF1, as described in the [[resources:eval:user-guides:ADE9430#Zero-Crossing Detection|Zero-Crossing Detection section]], and then using the two values near the positive to negative zero-crossing to calculate the exact zero-crossing point using linear interpolation. This information is used to precisely calculate the line period, which is stored in the xPERIOD register.
   \\
   \\
   {{ :resources:eval:user-guides:ade9430_trm:line_period_calculation_using_zero_crossing_detection_and_linear_interpolation.svg? 500 x 500 |}}
   <WRAP centeralign>**Figure 27. Line Period Calculation using Zero-Crossing Detection and Linear Interpolation**</WRAP>
   \\
   \\
   The line period, T<sub>L</sub>, can be calculated from the xPERIOD register according to the following equation:
   \\
   \\
   <m>T_L = {{xPERIOD + 1} / {8000 * 2^16} }(sec)</m>
   
   Similarly, the line frequency can be calculated from the xPERIOD register, using the following equation:
   \\
   \\
   <m>F_L = {{8000 * 2^16} / {xPERIOD + 1}}(Hz)</m>
   \\
   \\
   ...
   The line period is calculated for the Phase A, Phase B, and Phase C voltages and the combined voltage signal, as described in the [[resources:eval:user-guides:ADE9430#Combined Voltage Zero-Crossing|Combined Voltage Zero-Crossing section]], and stored in the APERIOD, BPERIOD, CPERIOD, and COM_PERIOD registers, respectively.
   ...
   \\
   ==== Angle Measurement ====
   The ADE9430 measures the time between zero-crossings on each phase. This measurement helps to determine if the system is balanced properly or to figure out if there was an installation error. The user can check if the phase angles correspond to the ones in the phasor diagrams in the [[resources:eval:user-guides:ADE9430#applying_the_ade9430_to_different_metering_configurations|Applying the ADE9430 to Different Metering Configurations section]].
   ...
   \\
   \\
   {{ :resources:eval:user-guides:ade9430_trm:voltage_to_voltage_phase_angle.svg? 500 x 500 |}}
   <WRAP centeralign>**Figure 28. Voltage to Voltage Phase Angle**</WRAP>
   \\
   \\
   ...
   \\
   \\
   {{ :resources:eval:user-guides:ade9430_trm:voltage_to_current_phase_angles.svg? 500 x 500 |}}
   <WRAP centeralign>**Figure 29. Voltage to Current Phase Angles**</WRAP>
   \\
   \\
   ...
   \\
   \\
   {{ :resources:eval:user-guides:ade9430_trm:4-wire_wye_and_4-wire_delta_normal_phase_sequence.svg? 500 x 500 |}}
   <WRAP centeralign>**Figure 30. 4-Wire Wye and 4-Wire Delta Normal Phase Sequence**</WRAP>
   \\
   \\
   For a 4-wire wye or 4-wire delta system, VCONSEL[2:0] is 000, 010, or 011, as described in the [[resources:eval:user-guides:ADE9430#Applying_the_ADE9430_to_Different_Metering_Configurations|Applying the ADE9430 to Different Metering Configurations section]]. In these 4-wire systems, the negative to positive transitions on ZXVA, ZXVB, and ZXVC are monitored to determine if there is a phase sequence error, as shown in (figure 32). To detect a phase sequence error, set how many sequences to observe in the SEQ_CYC register. It is recommended to set SEQ_CYC to 1. Figure 31 shows a phase sequence error for a 4-wire wye or 4-wire delta due to a wiring or installation error.
   \\
   \\
   {{ :resources:eval:user-guides:ade9430_trm:4-wire_wye_and_4-wire_delta_phase_sequence_error_wiring_error_.svg? 400 x 400 |}}
   <WRAP centeralign>**Figure 31. 4-Wire Wye and 4-Wire Delta Phase Sequence Error (Wiring Error)**</WRAP>
   \\
   \\
   ...
   \\
   \\
   {{ :resources:eval:user-guides:ade9430_trm:4-wire_wye_4-wire_delta_phase_sequence_error_from_a_phase_voltage_dropping_below_zxthrsh_with_seq_cyc_1.svg? 500 x 500 |}}
   <WRAP centeralign>**Figure 32. 4-Wire Wye, 4-Wire Delta Phase Sequence Error from a Phase Voltage Dropping Below ZXTHRSH with SEQ_CYC = 1**</WRAP>
   \\
   \\
   === 3-Wire Delta ===
   \\
   For a 3-wire delta system, VCONSEL[2:0] is 001 or 100, as described in the [[resources:eval:user-guides:ADE9430#Applying_the_ADE9430_to_Different_Metering_Configurations|Applying the ADE9430 to Different Metering Configurations section]]. In a 3-wire delta system, the ZXVC and ZXVA positive to negative and negative to positive transitions are monitored to detect a phase sequence error. Figure 33 shows the normal phase sequence for a 3-wire delta with VCONSEL[2:0] = 001.
   \\
   \\
   {{ :resources:eval:user-guides:ade9430_trm:3-wire_delta_normal_phase_sequence.svg? 500 x 500 |}}
   <WRAP centeralign>**Figure 33. 3-Wire Delta Normal Phase Sequence**</WRAP>
   \\
   \\
   ...
   \\
   \\
   {{ :resources:eval:user-guides:ade9430_trm:3-wire_delta_phase_sequence_error_wiring_error.svg? 500 x 500 |}}
   <WRAP centeralign>**Figure 34. 3-wire Delta Phase Sequence Error (Wiring Error)**</WRAP>
   \\
   \\
   ...
   \\
   \\
   {{ :resources:eval:user-guides:ade9430_trm:3-wire_delta_phase_sequence_error_from_a_phase_voltage_dropping_below_zxthrsh_with_seq_cyc_1.svg? 500 x 500 |}}
   <WRAP centeralign>**Figure 35. 3-Wire Delta Phase Sequence Error from a Phase Voltage Dropping Below ZXTHRSH with SEQ_CYC = 1**</WRAP>
   \\
   \\
   ...
   Alternatively, the user can set the number of samples used in the calculation by setting the UPERIOD_SEL bit in CONFIG2, where the user configured USER_PERIOD register is used instead of the selected line period measurement. For more information about USER_PERIOD and the line period measurements, see the [[resources:eval:user-guides:ADE9430#Line Period Calculation|Line Period Calculation section]].
   ...
   \\
   <WRAP center box 65%>
   <WRAP centeralign>**Table 18. ONE-CYCLE RMS Settling Time**</WRAP>
   ...
   <WRAP center box 60%>
   <WRAP centeralign>**Table 19. 10 Cycle RMS/12 Cycle RMS Settling Time**</WRAP>
   ...
   \\
   \\
   <m>APF = {AWATT accumulated {over} 1.024 sec} / {AVA accumulated {over} 1.024 sec}</m>
   \\
   \\
   ...
   \\
   \\
   {{ :resources:eval:user-guides:ade9430_trm:watt_and_var_sign_for_capacitive_and_inductive_loads.svg? 500 x 500 |}}
   <WRAP centeralign>**Figure 36. Watt and VAR Sign for Capacitive and Inductive Loads**</WRAP>
   \\
   \\
   ...
   Power Factor = xPF register × 2<sup>−27</sup>
   \\
   \\
   ==== Temperature ====
   ...
   \\
   \\
   {{ :resources:eval:user-guides:ade9430_trm:temperature_measurement_block_diagram.svg? 500 x 500 |}}
   <WRAP centeralign>**Figure 37. Temperature Measurement Block Diagram**</WRAP>
   \\
   \\
   ...
   \\
   \\
   The ADE9430 has a SPI-compatible interface, consisting of four pins: SCLK, MOSI, MISO, and SS. The ADE9430 is always a SPI subordinate; it never initiates SPI communication. The SPI interface is compatible with 16-bit and 32-bit read/write operations. See the [[resources:eval:user-guides:ADE9430#Register List|Register Information section]] for information about the length of each register.
   
   Figure 38. shows the connection between the ADE9430 SPI and a main device that contains a SPI interface.
   \\
   \\
   
   {{ :resources:eval:user-guides:connecting_the_ade9430_sub_spi_port_to_a_main_spi_device1.svg |}}
   <WRAP centeralign>**Figure 38. Connecting the ADE9430 Subordinate SPI Port to a Main SPI Device**</WRAP>
   \\
   \\
   ...
   \\
   \\
   {{ :resources:eval:user-guides:ade9430_trm:spi_read_protocol_example_crc_or_next_data_can_follow.svg? 500 x 500 |}}
   <WRAP centeralign>**Figure 39. SPI Read Protocol Example—CRC or Next Data can Follow**</WRAP>
   \\
   \\
   The default state of the MOSI pin depends on the main SPI device. Here, it is assumed to be high (Logic 1).
   \\
   \\
   {{ :resources:eval:user-guides:ade9430_trm:spi_write_protocol_example.svg? 500 x 500 |}}
   <WRAP centeralign>**Figure 40. SPI Write Protocol Example**</WRAP>
   \\
   \\
   ...
   \\
   \\
   {{ :resources:eval:user-guides:ade9430_trm:command_header_cmd_hdr.svg? 500 x 500 |}}
   <WRAP centeralign>**Figure 41. Command Header, CMD_HDR [15:0]**</WRAP>
   \\
   \\
   ...
   BURST_EN = 0 and the address is in the waveform buffer, 0x800 to 0xFFF, and BURST_CHAN is equal to 1111 (binary).
   The ADE9430 provides a SPI burst read functionality: instead of sending the CRC, the following data is from the next address if the following conditions apply see the [[resources:eval:user-guides:ADE9430#SPI Burst Read|SPI Burst Read section for more information]]:
   
   BURST_EN = 1, and the address is within the range of 0x500 to 0x63C or 0x680 to 0x6BC.
   The address is within the range of 0x800 to 0xFFF, and BURST_CHAN is not equal to 1111 (binary).
   ...
   **Table 20. Data Clocked Out After Addressed Data, in SPI Read Operation**
   ...
   \\
   \\
   {{ :resources:eval:user-guides:ade9430_trm:spi_read_protocol_example_where_the_following_data_is_the_crc_or_the_initial_data_is_repeated.svg? 500 x 500 |}}
   <WRAP centeralign>**Figure 42 SPI Read Protocol Example where the Following Data is the CRC or the Initial Data is Repeated**</WRAP>
   \\
   \\
   ...
   The CRC_SPI register value is appended to the 16-/32-bit data read from the register addressed in the CMD_HDR for the cases in Table 20 where CRC is written see the [[resources:eval:user-guides:ADE9430#SPI Read|SPI Read section]] for more information.
   ...
   \\
   \\
   {{ :resources:eval:user-guides:ade9430_trm:crc_calculation_of_32-bit_spi_data.svg? 500 x 500 |}}
   <WRAP centeralign>**Figure 43. CRC Calculation of 32-Bit SPI Data**</WRAP>
   \\
   \\
   {{ :resources:eval:user-guides:ade9430_trm:crc_calculation_of_16-bit_spi_data.svg? 500 x 500 |}}
   <WRAP centeralign>**Figure 44. CRC Calculation of 16-Bit SPI Data**</WRAP>
   \\
   \\
   {{ :resources:eval:user-guides:ade9430_trm:lfsr_generator_used_for_crc_spi_calculation.svg? 500 x 500 |}}
   <WRAP centeralign>**Figure 45. LFSR Generator Used for CRC_SPI Calculation**</WRAP>
   \\
   \\
   shows how the LFSR works. The MISO 32-bit data forms the [a<sub>31</sub>, a<sub>30</sub>,…, a<sub>0</sub>] bits used by the LFSR. Bit a<sub>0</sub> is Bit 24 of the first MISO 32-bit data to enter the LFSR, and the last data to enter the LFSR, Bit a31, corresponds to Bit 7 transmitted on MISO. The formulas that govern the LFSR are as follows.
   ...
   G(x) = x<sup>16</sup> + x<sup>12</sup> + x<sup>5</sup> + 1 (3)
   
   g<sub>0</sub> = g<sub>5</sub> = g<sub>12</sub> = 1 (4)
   
   All other g<sub>i</sub> coefficients are equal to 0.
   
   FB(j) = a<sub>j</sub> − 1 XOR b<sub>15</sub>(j − 1) (5)
   
   b<sub>0</sub>(j) = FB(j) AND g<sub>0</sub> (6)
   
   b<sub>i</sub>(j) = FB(j) AND gi XOR b<sub>i − 1</sub>(j − 1), i = 1, 2, 3, …, 15 (7)
   ...
   **Table 21. Order of Registers Included in Configuration Register CRC**
   ...
   The waveform buffer samples can be accessed using the SPI burst read functionality so that multiple samples can be read using only one SPI command header see the [[resources:eval:user-guides:ADE9430#Burst Read Waveform Buffer Samples from SPI|Burst Read Waveform Buffer Samples from SPI section]].
   ...
   **Table 23. 24-Bit Sinc4 and Sinc4 + IIR LPF Data**
   ...
   \\
   \\
   {{ :resources:eval:user-guides:ade9430_trm:fixed_data_rate_waveform_sample_storage.svg? 500 x 500 |}}
   <WRAP centeralign>**Figure 46. Fixed Data Rate Waveform Sample Storage**</WRAP>
   \\
   \\
   ...
   \\
   \\
   {{ :resources:eval:user-guides:waveform_buffer_page_arrangement_for_fixed_data_rate_samples_only.svg? 500 x 500 |}}
   <WRAP centeralign>**Figure 47. Waveform Buffer Page Arrangement—for Fixed Data Rate Samples Only**</WRAP>
   \\
   \\
   ...
   \\
   \\
   {{ :resources:eval:user-guides:ade9430_trm:resampled_waveform_sample_storage.svg? 500 x 500 |}}
   <WRAP centeralign>**Figure 48. Resampled Waveform Sample Storage**</WRAP>
   \\
   \\
   ...
   \\
   <WRAP center box 60%>
   <WRAP centeralign>**Table 25. Waveform Buffer Burst Read**</WRAP>
   ...
   **Table 26. SPI Address Interpretation when Reading from Waveform Buffer**
   ...
   \\
   \\
   {{ :resources:eval:user-guides:ade9430_trm:waveform_buffer_spi_burst_read_of_fixed_data_rate_samples_with_burst_chan_0_to_read_out_all_channels.svg? 900 x 900 |}}
   <WRAP centeralign>**Figure 49. Waveform Buffer SPI Burst Read of Fixed Data Rate Samples, with BURST_CHAN = 0, to Read Out All Channels**</WRAP>
   \\
   \\
   {{ :resources:eval:user-guides:ade9430_trm:waveform_buffer_spi_burst_read_of_resampled_data_with_burst_chan_0011.svg? 800 x 800 |}}
   <WRAP centeralign>**Figure 50. Waveform Buffer SPI Burst Read of Resampled Data, with BURST_CHAN = 0011, to Read out IC and VC Data**</WRAP>
   \\
   \\
   {{ :resources:eval:user-guides:ade9430_trm:waveform_buffer_spi_single_address_read_of_fixed_rate_data_with_burst_chan_1111.svg? 700 x 700 |}}
   <WRAP centeralign>**Figure 51. Waveform Buffer SPI Single Address Read of Fixed Rate Data with BURST_CHAN = 1111**</WRAP>
   \\
   \\
   {{ :resources:eval:user-guides:ade9430_trm:waveform_buffer_spi_single_address_read_of_resampled_data_with_burst_chan_1111.svg? 700 x 700 |}}
   <WRAP centeralign>**Figure 52. Waveform Buffer SPI Single Address Read of Resampled Data with BURST_CHAN = 1111**</WRAP>
   \\
   \\
   ...
   \\
   \\
   <m>V_a(t) = sqrt{2}sin(omega t)</m>
   \\
   \\
   <m>V_b(t) = sqrt{2}sin(omega t- 120^circ)</m>
   \\
   \\
   <m>V_c(t) = sqrt{2}sin(omega t+ 120^circ)</m>
   \\
   \\
   <m>i_a(t) = sqrt{2}sin(omega t- theta)</m>
   \\
   \\
   <m>i_b(t) = sqrt{2}sin(omega t- theta - 120^circ)</m>
   \\
   \\
   <m>i_c(t) = sqrt{2}sin(omega t- theta + 120^circ)</m>
   \\
   \\
   To understand how these signals relate to each other, create a phasor diagram following a convention using lagging phase angles. Figure 53 shows a common polyphase metering configuration, the 4-wire wye, with v<sub>a</sub>, v<sub>b</sub>, and v<sub>c</sub> defined in the previous equations. Phase B lags Phase A by 120°, and Phase C lags Phase A by 240°. Currents are shown at a power factor of 1, PF = 1, where θ = 0 in the i<sub>a</sub>, i<sub>b</sub>, and i<sub>c</sub> expressions (shown previously), and the current is in phase with the voltage.
   \\
   \\
   {{ :resources:eval:user-guides:ade9430_trm:4-wire_wye_service_vector_diagram.svg? 500 x 500 |}}
   <WRAP centeralign>**Figure 53. 4-Wire Wye Service Vector Diagram**</WRAP>
   \\
   \\
   ...
   \\
   \\
   {{ :resources:eval:user-guides:ade9430_trm:3-wire_delta_service_vector_diagram.svg? 550 x 550 |}}
   <WRAP centeralign>**Figure 54. 3-Wire Delta Service Vector Diagram**</WRAP>
   \\
   \\
   {{ :resources:eval:user-guides:4-wire_delta_service_vector_diagram.svg? 500 x 500 |}}
   <WRAP centeralign>**Figure 55. 4-Wire Delta Service Vector Diagram**</WRAP>
   \\
   \\
   {{ :resources:eval:user-guides:ade9430_trm:3-wire_residential_1ph_service_vector_diagram.svg? 500 x 500 |}}
   <WRAP centeralign>**Figure 56. 3-Wire Residential 1PH Service Vector Diagram**</WRAP>
   \\
   \\
   \\
   {{ :resources:eval:user-guides:ade9430_trm:3-wire_network_meter_vector_diagram.svg? 500 x 500 |}}
   <WRAP centeralign>**Figure 57. 3-Wire Network Meter Vector Diagram**</WRAP>
   \\
   \\
   The phasor diagrams help to understand how the voltages and currents are related in time. Figure 58 shows the 4-wire wye voltage phase sequence in time, corresponding to the Figure 53 phasor diagram and the equations for v<sub>a</sub>, v<sub>b</sub>, and v<sub>c</sub> provided previously.
   \\
   \\
   {{ :resources:eval:user-guides:ade9430_trm:4-wire_wye_voltage_phase_sequence_in_time.svg? 500 x 500 |}}
   <WRAP centeralign>**Figure 58. 4-Wire Wye, Voltage Phase Sequence in Time**</WRAP>
   \\
   \\
   ...
   \\
   <WRAP center box 100%>
   <WRAP centeralign>**Table 27. Non-Blondel Compliant Meter Forms**</WRAP>
   ...
   \\
   \\
   {{ :resources:eval:user-guides:ade9430_trm:4-wire_wye_neutral_connected_to_ground.svg? 500 x 500 |}}
   <WRAP centeralign>**Figure 59. 4-Wire Wye, Neutral Connected to Ground**</WRAP>
   \\
   \\
   Alternatively, a series impedance can be used on the neutral, as shown in Figure 60, which can be advantageous if an isolated power supply is used. Note that this configuration has poor performance if the phase voltages are not balanced. For more information, refer to the [[http://analog.com/media/en/technical-documentation/application-notes/AN-1334.pdf|AN-1334]]. For this configuration, VCONSEL[2:0] = 000.
   \\
   \\
   {{ :resources:eval:user-guides:ade9430_trm:4-wire_wye_series_impedance_on_the_neutral.svg? 500 x 500 |}}
   <WRAP centeralign>**Figure 60. 4-Wire Wye, Series Impedance on the Neutral**</WRAP>
   \\
   \\
   Phase sequence error detection is performed based on the expected ABC sequence; see the [[resources:eval:user-guides:ADE9430#Phase Sequence Error Detection|Phase Sequence Error Detection section]] for more information.
   ...
   \\
   \\
   {{ :resources:eval:user-guides:ade9430_trm:3-wire_delta_phase_b_connected_to_ground.svg? 500 x 500 |}}
   <WRAP centeralign>**Figure 61. 3-Wire Delta, Phase B Connected to Ground**</WRAP>
   \\
   \\
   ...
   \\
   \\
   {{ :resources:eval:user-guides:ade9430_trm:phasor_diagram_of_xv_pcf_and_xi_pcf_waveforms_inside_the_ic_with_3-wire_delta_with_phase_b_as_ground_and_vconsel_001.svg? 500 x 500 |}}
   <WRAP centeralign>**Figure 62. Phasor Diagram of xV_PCF and xI_PCF Waveforms Inside the IC with 3-Wire Delta with Phase B as Ground and VCONSEL = 001**</WRAP>
   \\
   \\
   ...
   \\
   \\
   {{ :resources:eval:user-guides:ade9430_trm:3-wire_delta_series_impedance_on_phase_b_and_vconsel_100_-_copy.svg? 500 x 500  |}}
   <WRAP centeralign>**Figure 63. 3-Wire Delta, Series Impedance on Phase B and VCONSEL = 100**</WRAP>
   \\
   \\
   ...
   \\
   \\
   {{ :resources:eval:user-guides:ade9430_trm:non-blondel_compliant_4-wire_wye.svg? 500 x 500 |}}
   <WRAP centeralign>**Figure 64. Non-Blondel Compliant 4-Wire Wye**</WRAP>
   \\
   \\
   The phasor diagram follows Figure 53. Phase sequence error detection is performed based on the expected ABC sequence; see the [[resources:eval:user-guides:ADE9430#Phase Sequence Error Detection|Phase Sequence Error Detection section]] for more information.
   ...
   \\
   \\
   {{ :resources:eval:user-guides:ade9430_trm:non-blondel_compliant_3-wire_delta.svg? 500 x 500 |}}
   <WRAP centeralign>**Figure 65. Non-Blondel Compliant 4-Wire Delta**</WRAP>
   \\
   \\
   The phasor diagram follows Figure 55. Phase sequence error detection is performed based on the expected ABC sequence; see the [[resources:eval:user-guides:ADE9430#Phase Sequence Error Detection|Phase Sequence Error Detection section]] for more information.
   ...
   **Table 28. Service Type and VCONSEL, ICONSEL Setting Summary**
   ...
   ====== Quick Start ======
   \\
   This section describes how to set up the ADE9430 for a 3-phase,
   4-wire wye measurement. Figure 59 shows the typical hardware
   connection for a 3-phase, 4-wire wye configuration.
   \\
   \\
     - Wait for the RSTDONE interrupt, indicated by the IRQ1 pin going low.
     - Configure PSM0 normal power mode by setting the PM1 pin and the PM0 pin to low.
     - Configure PGA gain on current and voltage channels using the PGA_GAIN gain register. The default gain on all channels is 1.
     - Configure the HPFDIS bits in the CONFIG0 register to enable/disable the high-pass filter. The high-pass filter is enabled by default. It is recommended to keep the high-pass filter enabled. Set the desired corner frequency for HPF using the HPF_CRN bits in the CONFIG2 register. The default value for HPF_CRN is 6 (1.25 Hz).
     - Configure the expected fundamental frequency using the SELFREQ bit (50 Hz: SELFREQ = 0, 60 Hz: SELFREQ = 1) in the ACCMODE register, and program the nominal voltage in the VLEVEL register for fundamental calculations. VLEVEL = X × 1,144,084, where X is the dynamic range that the nominal signal is at with respect to full scale.
     - Configure the zero-crossing source for ZX detection. If ZX_SRC_SEL = 1 in the CONFIG0 register, data before the HPF and phase compensation is used. If ZX_SRC_SEL = 0, data after the HPF and phase compensation is used. It is recommended to have ZX_SRC_SEL = 0.
     - Set VCONSEL = 000 in the ACCMODE register for a 3-phase, 4-wire wye configuration.
     - <WRAP> If energy is monitored using the CF outputs, configure the following registers. Skip this section if the CF outputs are not used.
       - Configure the CFxSEL bits in the CFMODE register to select the energy type to monitor.
       - Configure the TERMSELx bits in the COMPMODE register to select the phases to include in the CF calculation.
       - Program xTHR to 0x00100000.
       - Compute and program the corresponding CFxDEN register based on the desired impulses per kilowatt-hour.
       - Ordered List Item * Configure the CF pulse width using the CF_LCFG register.
      \\
    </WRAP>
     - <WRAP>If energy is monitored using energy registers, configure the following registers:
       - Configure the WATTACC and VARACC bits in the ACCMODE register to select amongst available accumulation modes (for example: signed, absolute, positive, or negative accumulation mode). The default accumulation mode is signed.
       - Configure the NOLOAD_TMR bits in the EP_CFG register and set the ACT_NL_LVL, REACT_NL_LVL, and APP_NL_LVL level registers to detect no load and prevent energy accumulation of noise.
       - Configure the EGY_TMR_MODE bit in the EP_CFG register to select sample (EGY_TMR_MODE = 0) or line cycle (EGY_TMR_MODE = 1) accumulation. Set the desired samples or half line cycles in the EGY_TIME register.
       - Configure the EGY_LD_ACCUM bit in the EP_CFG register to add the internal energy register to user energy register on EGYRDY (EGY_LD_ACCUM = 0), or to overwrite the user energy register with the internal energy register value (EGY_LD_ACCUM = 1).
       - Configure the RD_RST_EN bit in the EP_CFG register to enable reset of user energy registers on read (RD_RST_EN = 1), or to disable reset of user energy registers on read (RD_RST_EN = 0).
       - 
   </WRAP>
     - The ADE9430 can provide interrupts for a variety of events on the IRQ0 and IRQ1 pins. The MASK0 or MASK1 and STATUS0 or STATUS1 registers manage the respective interrupt pins.
     - See the [[resources:eval:user-guides:ADE9430#Power Quality Measurements|Power Quality Measurements section]] to configure the power quality parameters.
     - See the [[resources:eval:user-guides:ADE9430#Waveform Buffer|Waveform Buffer section]] to configure and use the waveform buffer.
     - Enable the DSP by setting the RUN register = 1, and enable energy accumulation by setting the EGY_PWR_EN bit in the EP_CFG register = 1.
     - Note that calibration is performed once at typical operating conditions. When the calibration values are computed, write the constants to registers before enabling the DSP.
   ...
   <WRAP center box 65%>
   <WRAP centeralign>**Table 1. Full-Scale ADC Codes**</WRAP>
   ...
   The voltage transfer function is 1/(990 + 1) = 0.001 V rms/A rms.
   ...
   Icalibration<sub>FSP</sub> × Full-Scale RMS Codes = 0.0002 × 52,702,092 = 10,540.
   ...
   APGAIN is the gain calibration register for Phase A. The active, reactive, and apparent powers in each phase have a common gain register and individual offset calibration registers.
     ...
     - Apply the nominal voltage and current at power factor = 1.
     - EP_CFG = 0x0011 and EGY_TIME = 7999 (1 sec accumulation).
     - Read the AWATTHR_HI register. 
     - The APGAIN register value is obtained as follows:
   AWATTHR_HI<sub>EXPECTED</sub> = I<sub>FSP</sub> × V<sub>FSP</sub> × Full-Scale Power Codes × Accumulation Time × 8000 × 2<sup>−13</sup>  =  0.0947 × 0.311 × 20,694,066 × 1 sec × 8000 × 2<sup>−13</sup> = 595,191
   \\
   \\
   Let AWATTHR_HI<sub>MEASURED</sub> = 580,000:
   \\
   \\
   APGAIN = (595,191 - 580,000) / 580,000 * 2<sup>27</sup> = 3515347 = 0x35A3D3
   \\
   \\
   ...
   \\
     - Apply a nominal voltage and offset calibration current at power factor = 1.
     - EP_CFG = 0x0013 and EGY_TIME = 1000 (10 sec accumulation).
     - Read the AWATTHR_HI register.
     - AWATTOS is calculated as
   \\
   AWATTHR_HI<sub>EXPECTED</sub> = Icalibration<sub>FSP</sub> × V<sub>FSP</sub> × Full-Scale Power Codes × Accumulation Time × 8000 × 2<sup>−13</sup> = 0.0002 × 0.311 × 20,694,066 × 10 sec × 8000 × 2<sup>−13</sup> = 12570
   
   To compute AVAROS, apply a nominal voltage and offset calibration current at power factor = 0.
   ...
   \\
   Conversion constants are used to convert ADE9430 register readings into physical parameters. When the device is calibrated to the full-scale ADC codes given in [[resources:eval:user-guides:ADE9430#Calibration Full-Scale Codes | Full-Scale codes table]], the conversion constants for the example system are calculated as follows:
   \\
   \\
   **Current Conversion Constant (uAmps/LSB)**
   \\
   \\
   ( Inominal * 10<sup>6</sup> ) / ( I% full-scale * Full Scale RMS codes ) 
   \\
   \\
   ( 10 Amps * 10<sup>6</sup> ) / ( 0.0947 * 52,702,092 = 2.0036 uARMS/LSB )
   \\
   \\
   **Voltage Conversion Constant (uVRMS/LSB)**
   \\
   \\
   ( Vnominal * 10<sup>6</sup> ) / ( V% full-scale * Full Scale RMS codes )
   \\
   \\
   ( 220  VRMS * 10<sup>6</sup> ) / ( 0.311 * 52,702,092 = 13.4225 uVRMS/LSB )
   \\
   \\
   **Power Conversion Constant  (mWATT/mVAR/mVA)/LSB**
   \\
   \\
   ( Inominal * Vnominal * 10<sup>3</sup> ) / ( I% full-scale * V% full-scale power codes )
   \\
   \\
   ( 10 ARMS * 220  VRMS * 10<sup>3</sup> ) / ( 0.0947 * 0.311 * 20,694,066 ) = 3.6097 (mWATT/mVAR/mVA) /LSB
   \\
   \\
   **Energy Conversion Constant (uWATTHr/uVARHr/uVAHr)/LSB**
   \\
   \\ 
   ( Inominal * Vnominal * 10<sup>6</sup> ) / ( I% full-scale * V% full-scale * full-scale power codes * 8000 * 2<sup>-13</sup> * 3600 )
   \\
   \\
   ( 10 ARMS * 220 VRMS * 10<sup>6</sup> ) / ( 0.0947 * 0.311 * 20,694,066 * 8000 * 2<sup>-13</sup> * 3600 ) = 1.0268 (uWATTHr/uVARHr/uVAHr) /LSB
   \\
   \\
   ...
   ^	0x4FE	|	Version	|	Version of ADE9430 IC.	|	0x00FE	|	R	|
   ...
   ^		|		|		|		|	0	|	xI_PCF waveforms, after the high-pass filter.	|		|		|
   ^		|		|		|		|	1	|	ADC samples, before the high-pass filter.	|		|		|
   ^		|		|	6	|	ZX_SRC_SEL	|		|	This bit selects whether data going into the zero-crossing detection circuit comes before the high-pass filter, and phase compensation or afterwards.	|	0x0	|	R/W	|
   ^		|		|		|		|	0	|	After the high-pass filter, and phase compensation.	|		|		|
   ^		|		|		|		|	1	|	Before the high-pass filter,  and phase compensation.	|		|		|
   ...
   ^		|		|		|		|		|	Bit 15. When OC_EN[3] is set to 1, Phase A is selected for the overcurrent detection.	|		|		|
   ^		|		|		|		|		|	Bit 14. When OC_EN[2] is set to 1, Phase B is selected for the overcurrent detection.	|		|		|
   ^		|		|		|		|		|	Bit 13. When OC_EN[1] is set to 1, Phase C is selected for the overcurrent detection.	|		|		|
   ^		|		|		|		|		|	Bit 12. When OC_EN[0] is set to 1, the neutral line is selected for the overcurrent detection.	|		|		|
