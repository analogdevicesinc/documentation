...
The quick start guide is available on the landing page here :doc:`(Quick Start Guide) </wiki-migration/resources/eval/user-guides/eval-ad4111ardz>` or for the step by step install guide see the below

Install Guide
-------------

The EVAL-AD4111ARDZ evaluation kit includes a link to the software that needs to be installed before using the EVAL-AD4111ARDZ evaluation board.
...
To start the ACE evaluation software, from the Windows Start menu, click Analog Devices > ACE. The software window opens (See Figure 5) until the software recognizes the AD4111 evaluation board. When the software recognizes the board, double-click on the icon in the **Start** view to open the main window seen in Figure 6. Make sure that you already have the AD411x plugin in the plugin manager.

By clicking on the part in the main ACE evaluation software window (See Figure 6), the chip view will be opened (Figure 7).

The chip view shows the block diagram of the AD4111. This tab allows the user to select inputs, set up the ADC, reset the ADC, and view errors present, as well as configure the device for different demonstration modes. Figure 7 shows the chip view in detail, and the following sections discuss the different elements on the Chip view of the software window.

|image2| **Figure 5. ACE Evaluation Select Interface Window**

|image3| **Figure 6. ACE Evaluation Software Main Window**

CONFIGURATION TAB
~~~~~~~~~~~~~~~~~

The **Configuration tab** shows a block diagram of the AD4111. This tab allows the user to select inputs, set up the ADC, reset the ADC, and view errors present, as well as configure the device.

Functional Block Diagram (1)
^^^^^^^^^^^^^^^^^^^^^^^^^^^^

-  The functional block diagram of the AD4111 (Label 1 in Figure 7) shows each of the functional blocks within the AD4111. Clicking a functional block on this diagram opens the configuration pop-up window for that block.
...
-  There are three external parameters that are set by the EVAL-AD4111ARDZ but must be entered into the software: the external reference (Label 3 in Figure 7), AVDD (Label 4 in Figure 7), and AVSS (Label 5 in Figure 7). The external reference on the EVAL-AD4111ARDZ is set to 2.5 V by using an ADR4525. If bypassing the ADR4525 on the board, change the external reference voltage value in the software to ensure correct calculation of results in the Waveform and Histogram tabs in the Waveform Analysis window.

Status Bar (6)
^^^^^^^^^^^^^^

-  The status bar (Label 6 in Figure 7) displays status updates.

Reset (7)
^^^^^^^^^

-  Click **Reset** to perform a software reset of the AD4111. There is no hardware reset pin on the AD4111. To perform a hard reset, remove power from the board. The software reset has the same effect as a hard reset.
...
.. image:: https://wiki.analog.com/_media/resources/eval/user-guides/eval-ad4111ardz/ad4111_functional_block_diagram.png
   :align: center

**Figure 7. AD4111 Functional Block Diagram**
...
-  Sets the input range of the ADC (Label 13 in Figure 8).
...
.. image:: https://wiki.analog.com/_media/resources/eval/user-guides/eval-ad4111ardz/ad4111_waveform_tab.png
   :align: center

**Figure 8. AD4111 Waveform Tab**
...
-  The plot selection control (Label 20 in Figure 9) selects which inputs display on the data waveform and shows the name of the input next to the on and off controls. These controls only affect the histogram graphs and have no effect on the channel settings in the ADC register map.
...
.. image:: https://wiki.analog.com/_media/resources/eval/user-guides/eval-ad4111ardz/ad4111_histogram_tab.png
   :align: center
...
-  The Bitfield Documentation (Label 28 in Figure 10) contains the documentation for the register or the bit field selected. This field can be updated by selecting a register or bit field in the register tree or hovering over the register or bit field in the register tree or register control. This documentation will be displayed by clicking the |image4| button (Label 28 in Figure 10).
...
.. image:: https://wiki.analog.com/_media/resources/eval/user-guides/eval-ad4111ardz/ad4111_memory_map.png
   :align: center

**Figure 10. Memory Map**

| :doc:`Return to Hardware Guide </wiki-migration/resources/eval/user-guides/eval-ad4111ardz/hardware_guide>`
...
.. |image1| image:: https://wiki.analog.com/_media/resources/eval/user-guides/eval-ad4111ardz/libiio_drivers.png
   :width: 400px
.. |image2| image:: https://wiki.analog.com/_media/resources/eval/user-guides/eval-ad4111ardz/ad4111_select_interface_window.png
.. |image3| image:: https://wiki.analog.com/_media/resources/eval/user-guides/eval-ad4111ardz/ad4111_ace_evaluation_software_main_window.png
.. |image4| image:: https://wiki.analog.com/_media/resources/eval/user-guides/eval-ad4111ardz/ad4111_documentation_button.png
   :width: 20px
