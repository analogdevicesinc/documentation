...
| 
| ====Evaluation Board Setup Procedures==== The EV-AD4131WASDZ-U1 connects to the SDP-B/ SDP-K1. The SDP-B/ SDP-K1 is the communication link between the PC and the EV-AD4131WASDZ-U1. ===Connecting the EV-AD4131WASDZ-U1 and the SDP to a PC==== After the ACE software is installed, take the following steps to set up the EV-AD4131WASDZ-U1 and the SDP.

-  Ensure that all configuration links are in the appropriate positions, as detailed in `hardware guide <http://wiki.analog.com/resources/eval-AD4131/hardwareguide>`__.
-  Connect the EV-AD4131WASDZ-U1 to the 160-way connector on the SDP. The EV-AD4131WASDZ-U1 does not require an external power supply adapter.
-  Connect the SDP to the PC via the USB cable included in the SDP kit.

===Verifying the Board Connection=== After connecting the power and the USB cable from the SDP to the PC, take the following steps to verify the board connection:
...
-  The **Computer Management** window opens. From the list labeled **System Tools**, click **Device Manager**. If the SDP driver is installed and the board is properly connected to the PC, **Analog Devices SDP-B** is shown in the **ADI Development Tools** list in the **Device Manager** window.

| ===Disconnecting the EV-AD4131WASDZ-U1=== Disconnect power from the SDP, or press the reset tact switch located alongside the mini USB port on the SDP, before removing the EV-AD4131WASDZ-U1 from the SDP. ====AD4131 Plugin Download==== Download the AD4130 Plugin software from your `myanalog <https://my.analog.com/en/app/>`__ account.
...
====AD4131 Plugin Install==== After the AD4131 Plugin is downloaded follow the steps to install the file:

-  Double click on the AD4131 Plugin.
-  Connect up your EV-AD4131-8ASDZ-U1/EV-AD4131WASDZ-U1/EV-AD4131WASDZ-U1 to your pc through a SDP-B controller board.

Alternatively, the AD4131 Plugin can be installed through the steps bellow:
...
-  Under the Name write “AD4131”.
-  Under Source hit the … button and set the path to where you have stored the AD4131 Plugin.
-  Hit “Ok”.
-  Hit “Close”.
...
After the EV-AD4131WASDZ-U1 and SDP-B/ SDP-K1 are properly connected to the PC, launch the :adi:`ACE <en/design-center/evaluation-hardware-and-software/evaluation-development-platforms/ace-software.html?doc=EVAL-AD7383FMCZ-ug-1770.pdf>` software by taking the following steps:

-  From the **Start** menu of the PC, select **All Programs > Analog Devices > ACE> ACE.exe** to open the ACE software main window.
-  If the EV-AD4131WASDZ-U1 is not connected to the USB port via the SDP-B when the software launches, the **AD4131 Eval Board** icon does not appear in the **Attached Hardware** section in ACE. To make the **AD4131 Eval Board** icon appear, connect the EV-AD4131WASDZ-U1 and the SDP to the USB port of the PC, wait a few seconds, and then follow the instructions in the dialog box that opens.
-  Double click the **AD4131 Eval Board** icon to open the **AD4131 Eval Board** view window.
-  Double click the **AD4131** chip icon in the **AD4131 Eval Board** view window to open the **AD4131** chip view window.
-  Click **Software Defaults** and then click **Apply Changes** to apply the default settings to the AD4131.
...
-  The “Proceed to Memory Map” button brings the user to the memory map of the AD4131. This allows the user to configure the AD4131.
-  The “Proceed to Analysis” button brings the user to the Analysis tab. This allows the user to see the performance results of the AD4131 and displays the data.
...
Use the Memory Map tab to access the registers of the AD4131-8. This tab changes register settings and shows additional information about each bit in each individual register.
