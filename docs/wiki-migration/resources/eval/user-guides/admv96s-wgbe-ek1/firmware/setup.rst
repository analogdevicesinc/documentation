...
`fw-update-bad.webm <https://wiki.analog.com/_media/resources/eval/user-guides/admv96s-wgbe-ek1/fw-update-bad.webm>`__
...
If what you see on your device is not the one indicated above, please follow the `instructions on this page <https://github.com/analogdevicesinc/max32625pico-firmware-images#how-to-update-the-firmware>`__ to update the MAXDAP firmware first.
...
`no-OS <https://github.com/analogdevicesinc/no-OS>`__ provides a standalone script that can be used to program .elf files to various targets, including the :adi:`MAX32650 <en/products/max32650.html>`. Typical usage and output is shown below:
...
The fact that the firmware exposes the on-board devices as iio devices in a `standardized way <https://www.kernel.org/doc/html/v4.12/driver-api/iio/index.html>`__ means that it is easy to devise new applications that can interact with the firmware by simply using the ``libiio`` library.
