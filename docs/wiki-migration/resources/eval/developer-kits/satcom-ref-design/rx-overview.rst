...
The `ADI Frequency Planning Utility tool <http://www.analog.com/en/design-center/interactive-design-tools/frequency-folding-tool.html>`__ was used to determine the optimal :adi:`AD9082` ADC sample rate, IF sampling frequency, and IF bandwidth. A 6GSPS ADC rate allows for maximum instantaneous bandwidth (IBW) and complete avoidance of HD2 aliasing in-band for a 1GHz signal bandwidth centered at 4.5GHz (2nd Nyquist). The figure below illustrates this.
