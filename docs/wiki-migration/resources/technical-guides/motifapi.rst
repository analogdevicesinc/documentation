...
| ``<motif>[command(s)]</motif>`` All received message content are children of the root element:
| ``<responses>[answers(s)]</responses>`` Sent messages are composed of a series of atomic messages called commands. Commands perform a specific task understood by MOTIF.dll. Each sent message solicits a response that is called an answer.
...
.. container:: indent
   ...
   .. container:: indent
      ...
      \ ``[dll version]`` – version of the MOTIF.dll.
