...
| 
| ===== Overview ===== The :adi:`MAX31865` is an easy-to-use resistance-to-digital converter optimized for platinum resistance temperature detectors (RTDs). An external resistor sets the sensitivity for the RTD being used and a precision delta-sigma ADC converts the ratio of the RTD resistance to the reference resistance into digital form.

The :adi:`MAX31865`\ ’s inputs are protected against overvoltage faults as large as Q45V. Programmable detection of RTD and cable open and short conditions is included.
...
.. image:: https://wiki.analog.com/_media/resources/eval/user-guides/max31865/max31865.png
   :align: center
   :width: 400px
...
.. admonition:: Download
   ...
   -  `MAX31865 Header file <http://analogdevicesinc.github.io/no-OS/max31865_8h.html>`__
   -  `MAX31865 Source file <http://analogdevicesinc.github.io/no-OS/max31865_8c.html>`__
...
| === Bias Configuration === When no conversions are being performed, VBIAS may be disabled to reduce power dissipation. When automatic (continuous) conversion mode is selected, VBIAS remains on continuously.
...
| === Conversion Mode Configuration === Conversion mode is configurable between automatic conversion mode where continuous conversion is used and "Normally Off" mode where 1-shot conversion is used, this also has a selectable 50/60Hz conversion rates.
...
.. note::
   ...
   | === Wire Configuration === MAX31865 can be configured to support 2-wire, 3-wire and 4-wire types of RTD sensors.
...
::
   ...
   max31865_remove(max31865_device);

   return 0;

|

.. |Header file of MAX31865 Driver| image:: https://wiki.analog.com/_media/:git-no-OS:`drivers/temperature/max31865pmb1/max31865`.h
.. |Implementation of MAX31865 Driver| image:: https://wiki.analog.com/_media/:git-no-OS:`drivers/temperature/max31865pmb1/max31865`.c
