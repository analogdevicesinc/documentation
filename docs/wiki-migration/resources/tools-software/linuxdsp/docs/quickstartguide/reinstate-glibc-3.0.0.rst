...
Although **``musl``** is a lot leaner than **``glibc``**, they are largely compatible. There is `a number of functional differences listed on wiki.musl-libc.org <https://wiki.musl-libc.org/functional-differences-from-glibc.html>`__ for further reading.
