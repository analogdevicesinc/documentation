...
-  Added `U-Boot Falcon Mode <https://github.com/u-boot/u-boot/blob/master/doc/README.falcon>`__ support for SC59x and :doc:`example </wiki-migration/resources/tools-software/linuxdsp/docs/quickstartguide/u-boot-falcon-mode>`
...
A: This is due to the jitter entropy source (software-based) that was added in rng-tools version 6. The initialization of it takes quite some time on smaller ARMs. There is a patch that fixes that, by moving the entropy generation sources so that hardware RNG is always used (through ``/dev/hwrng``). You can apply that from the develop/3.1.0 branch `Placeholder link <https://github.com>`__
