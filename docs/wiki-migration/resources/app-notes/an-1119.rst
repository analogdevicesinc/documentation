...
| |an1119-image001_110dpi.png| *Figure 1. Typical Switching Regulator (Showing AC and DC Current Paths)*

Switch current paths, such as DH, DL, BST, and SW, come off the controller and need proper placement to avoid excessive parasitic inductance. These lines carry high :math:`\delta` I/ :math:`\delta` t ac switching pulse currents that can reach more than 3 A for a few nanoseconds. Keeping the high current loops small is critical to avoid additional noise pickup and minimize output ringing.
...
|an1119-image003.png| *Figure 3. Cross-Section View: Connecting PGND Planes to Improve Heat Dissipation*

This multiground plane approach insolates noise sensitive signals. As shown in Figure 2, the negative terminals of compensation components, the soft start capacitor, the bias input bypass capacitors, and the output feedback divider resistors all tie to an AGND plane. Do not directly connect any high current or high :math:`\delta` I/ :math:`\delta` t paths to the isolated AGND plane. The AGND is a quiet ground plane with no large currents flowing through it.
...
|an1119-image004.png| //Figure 4. AGND Plane-to-PGND Plane Connection //
...
The :adi:`ADP1850` dual channel step-down controller uses the R\ :sub:`DS(ON)` of the low-side MOSFET as part of the control loop architecture. This architecture senses the current through the low-side MOSFET between the SWx and PGNDx pins. Ground current noise in one channel can couple into the adjacent channel. Therefore, it is essential to keep the SWx and PGNDx traces as short as possible and place them close to the MOSFETs for accurate current sensing. Be sure the connections to the SWx and PGNDx nodes implement the Kelvin sensing technique, as illustrated in Figure 2 and Figure 5. Note that the respective PGNDx trace connects to the source of the low-side MOSFET. Do not arbitrarily connect the PGND plane to the PGNDx pin.

In contrast, for dual channel voltage mode controllers, such as the :adi:`ADP1829`, the PGND1 and PGND2 pins directly tie to the PGND plane with vias.

|an1119-image005.png| //Figure 5. Grounding Technique for Two Channels //

Feedback and Current-Limit Sense Paths
--------------------------------------

The feedback (FB) and current-limit (ILIM) pins are low signal level inputs; therefore, they are sensitive to capacitive and inductive noise pickup. Avoid running FB and ILIM traces close to high :math:`\delta` I/ :math:`\delta` t traces. Watch out for trace routings that form loops that increase unwanted inductance. Adding a small MLCC decoupling capacitor, for example, 22 pF, between ILIM and PGND helps with additional noise filtering.
...
Applications that are more sensitive to electromagnetic interference, switch node noise, and ringing can take advantage of a small snubber. A snubber is a resistor in series with a capacitor (see R\ :sub:`SNUB` and C\ :sub:`SNUB` in Figure 6). The snubber that is placed between the SW node and the PGND plane reduces ringing on the SW node and provides a reduced level of EMI. Keep in mind that this addition can slightly reduce the overall efficiency by 0.2% to 0.4%.

|an1119-image006.png| *Figure 6. Snubber and Gate Resistance Circuit*

Gate Driver Paths
-----------------

Gate drive traces (DH and DL) also handle high :math:`\delta` I/ :math:`\delta` t and tend to produce ringing and overshoot. These traces should be as short as possible. It is best to route them directly, avoid using feedthrough vias. If vias are necessary, use two vias for each one to reduce the peak current density and parasitic inductance.

Slowing the gate drive with a small resistor (approximately 2 :math:`\Omega` to 4 :math:`\Omega` ) in series with the DH or DL pins also mitigates gate noise and overshoot. Another option is a resistor between the BST and SW pins (see Figure 6). Reserving the space with 0 :math:`\Omega` gate resistors during layout adds great flexibility later during evaluation. The added gate resistance increases the gate charge rise and fall times and thus increases the switching power loss in the MOSFET.

Summary
-------

Understanding the current paths, their sensitivities, and proper placement of the components is key to avoiding noise problems in PCB layout designs. All Analog Devices, Inc. power component evaluations boards implement the presented layout guidelines for best performance. Evaluation board documents, :adi:`UG-204` and :adi:`UG-205`, provide detail layouts specific to the :adi:`ADP1850`.

Note that all switching power supplies have common components and similar current path sensitivities. Therefore, the guidelines highlighted in the :adi:`ADP1850` example for a current mode step-down regulator applies directly to voltage mode and/or step-up switching regulator layouts.

.. |an1119-image001_110dpi.png| image:: https://wiki.analog.com/_media/resources/app-notes/an1119-image001_110dpi.png
.. |an1119-image002_110dpi.png| image:: https://wiki.analog.com/_media/resources/app-notes/an1119-image002_110dpi.png
.. |an1119-image003.png| image:: https://wiki.analog.com/_media/resources/app-notes/an1119-image003.png
.. |an1119-image004.png| image:: https://wiki.analog.com/_media/resources/app-notes/an1119-image004.png
.. |an1119-image005.png| image:: https://wiki.analog.com/_media/resources/app-notes/an1119-image005.png
.. |an1119-image006.png| image:: https://wiki.analog.com/_media/resources/app-notes/an1119-image006.png
