...
This document presents the steps to setup an environment for using the :adi:`EVAL-AD7687SDZ <AD7687>` evaluation board together with the Xilinx KC705 FPGA board and the Xilinx Embedded Development Kit (EDK). Below is presented a picture of the EVAL-AD7687SDZ Evaluation Board with the Xilinx KC705 board.
...
The EVAL-AD7687SDZ evaluation board is a member of a growing number of boards available for the SDP. It was designed to help customers evaluate performance or quickly prototype new AD7687 circuits and reduce design time. When using this evaluation board with the SDP board or Xilinx KC705 board, apply +7.5V as +Vs, a voltage between -2V and -5V as -Vs and +2.5V as VDD.
...
-  `Reference Design Files <https://wiki.analog.com/_media/resources/fpga/xilinx/interposer/ad7687_evalboard.zip>`__
...
+-------------+-------------------------------------------------------------------------------------------------------+
| Hdl         | Contains the HDL driver for the AD7687 ADC.                                                           |
+-------------+-------------------------------------------------------------------------------------------------------+
