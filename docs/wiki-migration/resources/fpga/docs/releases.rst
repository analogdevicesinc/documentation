...
All HDL pre-built files are provided to download together with Linux device trees and Kernel images, as a `boot partition archive <https://swdownloads.analog.com/cse/boot_partition_files/2023_r2/latest_boot.txt>`__ (just replace '2023_r2' in above link with other release name to get specific boot files, ex. 2022_r2). All boot files are also part of released :doc:`Kuiper images </wiki-migration/resources/tools-software/linux-software/adi-kuiper_images/release_notes>`.

Check :git-hdl:`GitHub Release Notes <releases>` for each release to see the main changes and links to related documentation.

On the other side, the 'main' HDL branch is used for development and regression testing is ran less often on it. This implies that it may not be always stable. Latest pre-build boot files from main are available `here <https://swdownloads.analog.com/cse/boot_partition_files/main/latest_boot.txt>`__.
