...
This lab presents the steps to setup an environment for using the :adi:`EVAL-AD7982SDZ` evaluation board together with the `BeMicro SDK <https://www.intel.com/content/www/us/en/programmable/b/bemicro-sdk.html>`__ USB stick and the Nios II Embedded Development Suite (EDS). Below is presented a picture of the EVAL-AD7982SDZ Evaluation Board with the BeMicro SDK Platform.

.. image:: https://wiki.analog.com/_media/resources/fpga/altera/bemicro/ad7982_bemicro.jpg
   :alt: ad7982_bemicro.jpg
   :align: center
   :width: 400px
...
.. image:: https://wiki.analog.com/_media/resources/fpga/altera/bemicro/ad7982_sdp1z.jpg
   :alt: ad7982_sdp1z.jpg
   :align: center
   :width: 400px

The **EVAL-AD7982SDZ** evaluation board is a member of a growing number of boards available for the **SDP**. It was designed to help customers evaluate performance or quickly prototype new **AD7982** circuits and reduce design time. When using this evaluation board with the SDP board or BeMicro SDK board, apply +7.5V as +Vs, a voltage between -2V and -5V as -Vs and +2.5V as VDD.

The :adi:`AD7982` is an 18-bit, successive approximation, analog-to-digital converter (ADC) that operates from a single power supply, VDD. It contains a low power, high speed, 18-bit sampling ADC and a versatile serial interface port. On the CNV rising edge, the AD7982 samples the voltage difference between the IN+ and IN- pins. The voltages on these pins usually swing in opposite phases between 0 V and VREF. The reference voltage, REF, is applied externally and can be set independent of the supply voltage, VDD. Its power scales linearly with throughput. The SPI-compatible serial interface also features the ability, using the SDI input, to daisy-chain several ADCs on a single 3-wire bus and provides an optional busy indicator. It is compatible with 1.8 V, 2.5 V, 3 V, and 5 V logic, using the separate VIO supply.

More information
----------------

-  :adi:`AD7982 Product Info <AD7982>` - pricing, samples, datasheet
-  `EVAL-AD7982SDZ evaluation board user guide <https://wiki.analog.com/_media/resources/fpga/altera/bemicro/user_guide_eval_10lead_pulsar.pdf>`__
...
-  `Lab Design Files <https://wiki.analog.com/_media/{{/resources/fpga/altera/bemicro/ad7982_evalboardlab.zip>`__

Extract the Lab Files
---------------------

Create a folder called “\ **ADIEvalBoardLab**\ ” on your PC and extract the **ad7982_evalboardlab.zip** archive to this folder. Make sure that there are **NO SPACES** in the directory path. After extracting the archive the following folders should be present in the **ADIEvalBoardLab** folder: **FPGA**, **Hdl** **Software**, **DataCapture**, **NiosCpu**.
...
|             | The *ip* subfolder contains the AD7982 NIOS II peripheral's source code.                                                                                                                                                                                                                 |
+-------------+------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------+
| Hdl         | Contains the source files for the AD7982 HDL driver:                                                                                                                                                                                                                                     |
...
+-------------+------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------+
| NiosCpu     | Contains the Quartus evaluation project source files . The *ip* subfolder contains the AD7982 Nios2 peripheral source code.                                                                                                                                                              |
+-------------+------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------+
...
.. image:: https://wiki.analog.com/_media/resources/fpga/altera/bemicro/ad7982_block_diagram.png
   :alt: HDL driver block diagram
   :align: center

Table 2 describes the ports of the AD7982 HDL driver.
...
+---------------------------------------------+-----------+-------+------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------+
| DATA_RD_READY_O                             | OUT       | 1     | Active high signal to indicate the status of a read operation from the AD7982. The IP continuously reads the conversion results from the ADC and outputs them on the DATA_O bus. When this signal is high data can be read from the DATA_O bus.                              |
+---------------------------------------------+-----------+-------+------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------+
| **AD7982 control and data ports**           |           |       |                                                                                                                                                                                                                                                                              |
+---------------------------------------------+-----------+-------+------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------+
...
+---------------------------------------------+-----------+-------+------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------+
| **Table 2 AD7982 driver ports description** |           |       |                                                                                                                                                                                                                                                                              |
+---------------------------------------------+-----------+-------+------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------+

The follwing figure presents the timing diagram for the read operations from the AD7982 driver.
...
|                                                     |        |       |        | Bit 3 is used to write data to the AD7982 evaluation board                                                  |
+-----------------------------------------------------+--------+-------+--------+-------------------------------------------------------------------------------------------------------------+
...
\* Execute **data_capture.bat** script. At this point 32 Kbyte of data will be acquired from the ADC and saved into the BeMicro SDK memory. The data stored in the BeMicro SDK memory is transfered to the PC. After the data is transferred to the PC it is converted to 18 bit values.
