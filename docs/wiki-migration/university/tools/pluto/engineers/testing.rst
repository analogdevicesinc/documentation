...
-  get the jig ready for the first batch:
   ...
   -  plug in all 4 USB `LabelWriter® 450 Turbo <http://www.dymo.com/en-US/labelwriter-450-turbo-label-printer>`__, into the USB A, B, C and D connectors on the side of the unit.
...
-  on the Pi, `dfu-utils <http://dfu-util.sourceforge.net/>`__ are used program the flash with the two files:
...
-  the label is printed via `glabels <http://glabels.org/>`__
