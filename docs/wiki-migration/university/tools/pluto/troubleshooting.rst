...
Here are some common solutions to problems users run into when setting up or using PlutoSDR. If you issue is not address please create a post in the :ez:`Virtual Classroom subforum <university-program>`. Search in the `EngineerZone <ez>>`__ forums is also a great resource.
...
-  `Main PlutoSDR documentation <https://wiki.analog.com/plutosdr>`__
...
-  **Linux only if libIIO installed from package manager:** Install `libiio-utils <https://packages.ubuntu.com/xenial/libs/libiio-utils>`__
