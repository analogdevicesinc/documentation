...
Analog Devices offers a variety of `No-OS drivers <https://github.com/analogdevicesinc/no-os>`__ for a variety peripherals in a hardware agnostic format. These drivers can be interfaced with the libm2k in order to use the M2K as a master to configure/use the peripheral. Documentation for the drivers available :doc:`here </wiki-migration/resources/no-os/drivers>`.
