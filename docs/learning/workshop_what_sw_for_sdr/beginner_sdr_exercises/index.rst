...
.. important::
   ...
   **Linux / Windows setup only:** download the exercise files before starting —
   :download:`beginner_exercises.zip <beginner_exercises.zip>` — and extract the archive to
   your Desktop to follow along.

   With the **Linux Kuiper** setup you do **not** need this
...
.. tab-set::
   ...
   .. tab-item:: Linux Kuiper

      With Kuiper Linux you get the complete environment in three steps, with no manual
      cloning required. This is the recommended setup.

      #. Flash your SD card with the **newest Kuiper Linux image** (Debian trixie) built by
         the official `ADI Kuiper repository <https://github.com/analogdevicesinc/adi-kuiper-gen>`__,
      ...
      #. Install the workshop package. It pulls in the SDR software stack (GNU Radio with
         ...
         .. note::
            ...
            directly, then pull in any missing dependencies:

            .. code-block:: bash

               sudo dpkg -i ./datax-workshops-what-sw-for-my-sdr_*.deb
               sudo apt install -f

      #. Make the packaged ``setup`` script executable, then run it to finish provisioning.
         Run it as your normal user (it uses ``sudo`` internally where needed):

         .. code-block:: bash

            sudo chmod +x /usr/share/datax-workshops-what-sw-for-my-sdr/setup
...
.. tab-set::
   ...
   .. tab-item:: Linux (Kuiper or Ubuntu)
      ...
      On a Linux host (including Kuiper / Debian trixie), the Pluto's default **RNDIS** mode
      may not come up, so the interface never appears and ``192.168.2.1`` is unreachable.
      Switch the Pluto to **NCM** (CDC-NCM), which Linux hosts support:
...
#. In GNU Radio Companion, open **console_message_receiver_pluto.grc** from
   **beginner_exercises → 2. Quadrature Phase Shift Keying (QPSK) → Console Messaging → Pluto → receiver_pluto**.
...
#. In GNU Radio Companion, open **console_message_receiver_jupiter.grc** from
   **beginner_exercises → 2. Quadrature Phase Shift Keying (QPSK) → Console Messaging → Jupiter → receiver_jupiter**.
...
#. In GNU Radio Companion, open **QPSK_raw_loopback_pluto.grc** from
   **beginner_exercises → 2. Quadrature Phase Shift Keying (QPSK)**.
...
#. In GNU Radio Companion, open **constellation_modulator_loopback_pluto.grc** from
   **beginner_exercises → 2. Quadrature Phase Shift Keying (QPSK) → Constellation Modulator**.
...
#. In GNU Radio Companion, open **FLL_loopback_pluto.grc** from
   **beginner_exercises → 2. Quadrature Phase Shift Keying (QPSK) → Frequency Locked Loop (FLL)**.
...
#. In GNU Radio Companion, open **ASK_loopback_pluto.grc** from
   **beginner_exercises → 3. Amplitude Shift Keying (ASK)**.
...
#. In GNU Radio Companion, open **doppler_radar.grc** from
   **beginner_exercises → 4. Doppler Radar**.
...
#. In GNU Radio Companion, click **File → Open** and open from **beginner_exercises → 4. Doppler Radar**
...
#. In GNU Radio Companion, open the **paint_tx.grc** file from
   **beginner_exercises → 5. Spectrum Paint**.
