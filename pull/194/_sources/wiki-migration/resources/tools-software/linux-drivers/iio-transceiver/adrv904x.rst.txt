.. warning:: Conversion failed for ``resources/tools-software/linux-drivers/iio-transceiver/adrv904x``

   Reason: pandoc error: Error at "/tmp/tmpfsc14bx4.txt" (line 43, column 1):
expecting inline
not found
| [[https://github.com/analogdevicesinc/linux/tree/koror_dev/drivers/iio/adc/koror/adrv904x.c|git]] | No |
^
...
.. code-block:: text
   ...
   ^ Source ^ Mainlined? ^'
   | [[repo>linux/tree/koror_dev/drivers/iio/adc/koror/adrv904x.c|git]] | No |
   
   ===== Files =====
   
   ^ Function ^ File ^
   | driver source file | [[repo>linux/tree/koror_dev/drivers/iio/adc/koror/adrv904x.c | drivers/iio/adc/koror/adrv904x.c]] |
   | driver source file | [[repo>linux/tree/koror_dev/drivers/iio/adc/koror/adrv904x_conv.c | drivers/iio/adc/koror/adrv904x_conv.c]] |
   | driver include | [[repo>linux/tree/koror_dev/drivers/iio/adc/koror/adrv904x.h | drivers/iio/adc/koror/adrv904x.h]] |
   | Koror API driver | [[repo>linux/tree/koror_dev/drivers/iio/adc/koror | drivers/iio/adc/koror]] |
   ...
   ==== Interrelated Device Drivers ====
     ...
     * [[resources:fpga:peripherals:jesd204|JESD204 Interface Framework]]
   ...
   | include | [[linux.github>master?drivers/iio/frequency/cf_axi_dds.h | drivers/iio/frequency/cf_axi_adc.h]] |
   
   === AXI JESD204B HDL driver ===
   
   ^ Function ^ File ^
   | driver  | [[linux.github>master?drivers/iio/jesd204/axi_jesd204_rx.c| drivers/iio/jesd204/axi_jesd204_rx.c]] |
   | driver  | [[linux.github>master?drivers/iio/jesd204/axi_jesd204_tx.c| drivers/iio/jesd204/axi_jesd204_tx.c]] |
   
   === AXI JESD204B GT (Gigabit Tranceiver) HDL driver (XILINX/ALTERA-INTEL) ===
   
   ^ Function ^ File ^
   | driver  | [[linux.github>master?drivers/iio/jesd204/axi_adxcvr.c | drivers/iio/jesd204/axi_adxcvr.c]] |
   
   ====== Device Driver Customization ======
   
   Please follow the link here for detailed options and examples:
   
     * [[:resources/tools-software/linux-drivers/iio-transceiver/adrv904x-customization|ADRV904x Device Driver Customization]]
   ...
   A stream processor is a processor within the transceiver tasked with performing a series of configuration tasks based on some event. After a request from the user, the stream processor performs a series of predefined actions that are loaded into the stream processor during
   device initialization. This processor takes full advantage of the speed of the internal register buses for efficient execution of commands.
   ...
   ^ Function ^ File ^
   | Steam | [[linux.github>koror_dev/firmware/stream_image.bin | firmware/stream_image.bin ]] |
   
   ===== ARM Processor =====
   
   The transceiver is equipped with two ARM M4 processors, CPU0 and CPU1, for Radio functionality. There is a separate A55 processor dedicated DFE features, like DPD, CLGC, VSWR. This section outlines ARM processor used for Radio features. The firmware for these ARM processors is loaded during the initialization process. CPU0 is loaded first, followed by CPU1 then CPU0 is started which will then start CPU1. The firmware memory size is 641 kB. The ARMs are tasked with configuring the transceiver for the selected use case,
   performing initial calibrations of the signal paths and maintaining device performance over time through tracking calibrations.
   ...
   This is the pre-compiled firmware binary for the embedded dual core ARM processors in the ADRV904x
   transceiver, which mainly consists of ADI proprietary algorithms used to calibrate the transceiver. It is delivered as part of ADRV904x software package.
   
   ^ Function ^ File ^
   | ARM processor firmware | [[linux.github>koror_dev/firmware/ADRV9025_DPDCORE_FW.bin | firmware/ADRV9040_FW.bin ]] |
   
   ===== DFE Processor =====
   
   The DFE processor is an ARM A55 quad-core processor for implementing DFE application software such as
   DPD, CLGC and VSWR algorithms.
   ...
   This is the pre-compiled firmware binary for the embedded quad core A55 DFE processors in the ADRV904x
   transceiver, which mainly consists of ADI proprietary DFE algorithms such as DPD, CLGC and VSWR.
   
   ^ Function ^ File ^
   | DFE processor firmware  | [[linux.github>koror_dev/firmware/ADRV9040_DFE_CALS_FW.bin | firmware/ADRV9040_DFE_CALS_FW.bin ]] |
   ...
   This is the front end gain look up tables for the ADRV904x receiver. It is a default table delivered as
   part of ADRV904x software package. User can also generate custom gain tables.
   
   adi,rx-gaintable-names = "ADRV9040_RxGainTable.csv";
   
   ^ Function ^ File ^
   | RX Gain Correction table | [[linux.github>koror_dev/firmware/ADRV9040_RxGainTable.csv | firmware/ADRV9040_RxGainTable.csv ]] |
   
   ===== Profile Binary ======
   
   The ADRV904x’s configuration for a particular use case is programmed through the profile binary. The
   profile consists of the filter coefficients, clock rates, signal blocks, DFE resources to enable/disable for a particular use case.
   
   This is a user generated with ADI evaluation software. Please refer to ADRV904x Evaluation System User
   Guide for steps to generate the profile binary.
   
   adi,device-config-name = "DeviceProfileTest.bin";
   
   ^ Function ^ File ^
   | RX Gain Correction table | [[linux.github>koror_dev/firmware/DeviceProfileTest.bin | firmware/DeviceProfileTest.bin ]] |
   ...
   __**General attribute naming convention:**__\\
   ...
   The ADRV904x driver supports advanced debug controls via the kernel [[wp>debugfs]]. These controls are mostly to debug which settings are currently configured in the device.  How these device files/controls can be used is described here.
   ...
   Pseudorandom Binary Sequence ([[wp>Pseudorandom_binary_sequence|PRBS]]) to Framer 0.
   ...
   **Example**: Digital TX -> Digital RX 
   ...
   User selectable tone (with frequency and gain selection), that can be injected into the TX path.
   Tx7 channel is selected.
